...
signed by *this* key." Here's exactly how the thumbprint is built and how to compute one with the
...
one up, but then two parties might disagree. RFC 7638 defines a way to **derive** the identifier from the
key itself, so it's always the same for the same key.
...
1. **Keep only the required members.** Each key type has a fixed set:
   - **RSA** → `e`, `kty`, `n`
   - **EC** → `crv`, `kty`, `x`, `y`
   - **OKP** (Ed25519 etc.) → `crv`, `kty`, `x`
   - **oct** (symmetric) → `k`, `kty`

   Everything else — `kid`, `use`, `alg`, and any private members — is dropped.
2. **Sort the members** into lexicographic (alphabetical) order.
3. **Serialize as compact JSON** — no whitespace, no extra characters.
4. **Hash** the UTF-8 bytes with **SHA-256**.
5. **base64url-encode** the digest, with no padding.

For an EC key, step 3 produces exactly:

```
{"crv":"P-256","kty":"EC","x":"f83OJ3D2xF1Bg8vub9tLe1gHMzV76e8Tus9uPHvRVEU","y":"x_FEzRu9m36HLN_tue659LNpXW6pCyStikYjKIWI5a0"}
```

…and hashing that yields the thumbprint. The calculator shows this canonical string so you can see
...
- **`kid` in a JWKS** — the most common use, so tokens can point at the signing key.
- **OAuth / OIDC key matching** and key rotation.
- **DPoP** and other JOSE proof-of-possession flows.
- **RFC 9278 thumbprint URI** — `urn:ietf:params:oauth:jwk-thumbprint:sha-256:<thumbprint>` — a standard
  URI form the calculator also generates.
