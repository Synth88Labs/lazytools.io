...
So `30 9 * * 1-5` reads: **minute 30, hour 9, any day of month, any month, days 1-5 (Mon, Fri)**. I.e. 09:30 every weekday. The two `*`s mean "every", which is how you say "don't constrain this
field."
...
Because a wrong cron line often fails silently, the job just runs at the wrong time or too often, verify the expression before it reaches production rather than waiting to see what happens:
