...
Whatever survives steps 1-3 with consistent weirdness, a stray prefix, doubled commas, an old domain, is a find & replace job. The replacement count is the safety net: 0 means your search text has a typo;
...
Structure first, then dedupe (with both ignore options on), then natural sort, then pattern-fixes, four clicks across [remove line breaks](/text/remove-line-breaks/),
