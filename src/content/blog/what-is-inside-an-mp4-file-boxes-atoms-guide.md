...
table. If you inspect a streaming segment and see `moof` boxes instead of one big `moov`, that's expected, it's a fragmented file, not a broken one.

## One structure, many extensions

Because MP4, **MOV**, **M4A**, **M4V** and **[HEIF/HEIC](/blog/heic-to-jpg-guide/)** all sit on ISOBMFF, they share this box layout, the same `ftyp`/`moov`/`mdat` grammar, just different brands and handler types. A `.mov` from a camera and
