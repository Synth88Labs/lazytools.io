...
Not every "encrypted" PDF is equally safe. The PDF standard has carried several encryption schemes
over the decades, and older ones are weak by modern standards. When you protect a file today you
want AES; when you receive an old protected file, it may be using something far more brittle.
...
password drawn from lowercase letters, uppercase, digits and symbols has roughly 95 possible
characters per position, so length compounds fast.
