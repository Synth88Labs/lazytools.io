...
names. XRechnung permits both; ZUGFeRD uses CII. You generally don't need to care which you have, a viewer that handles both just shows you the invoice.
...
PDF from stale data, or the two diverge for any reason, your accounting software processes the XML, so it's worth actually seeing what the XML says, not just trusting the picture.
