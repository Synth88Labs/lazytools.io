...
archetype: explainer
tools: ["/calendar/bikram-sambat-converter/", "/calendar/calendar-converter/", "/time/days-between-dates/"]
keywords:
  - bs to ad
  - ad to bs
...
Because the offset isn't a whole number of years, the exact difference shifts across the Gregorian
year. From roughly mid-April (Nepali new year) to the end of December, the BS year is **57** ahead of
...
Those lengths are worked out by astronomical calculation and published each year in official almanacs
(*patro*). This is precisely why a Nepali date converter can't be a few lines of arithmetic the way a
Gregorian leap-year check can: it has to consult a **table of real month lengths** for each BS year.
Good converters embed a validated table covering roughly **BS 1975 to 2099** (about AD 1918 to 2043).
...
The twelve months, from the new year:

| # | Month | Rough Gregorian span |
|---|---|---|
...
- **AD → BS:** pick the Gregorian date and the [converter](/calendar/bikram-sambat-converter/) returns
...
*Related tools: [calendar converter](/calendar/calendar-converter/) (Hijri, Hebrew, Persian and more)
· [days between dates](/time/days-between-dates/). Conversion uses validated open Bikram Sambat
calendar data ([source](https://github.com/remotemerge/nepali-date-converter)).*
