...
handling of things like time zones and multiple addresses. The `TYPE` parameter labels each value, `TEL;TYPE=CELL`, `EMAIL;TYPE=WORK`, `ADR;TYPE=HOME`, so an app knows which number is the mobile and
...
The conversion has to unfold wrapped lines, decode vCard escaping, and quote CSV values with commas, the same class of details as the `.ics` format.
