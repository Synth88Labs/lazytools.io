...
all: the [KSeF viewer](/file/ksef-viewer/) parses FA(3)/FA(2) XML into a normal, readable invoice, entirely in your browser.
...
## FA(2) vs FA(3): what actually changed

If you already handled the previous structure, most of your mental model carries over, the root
`<Faktura>`, the two `Podmiot` blocks, the `Fa` body and repeating `FaWiersz` lines are all still
there. FA(3) tightened and extended, rather than rebuilt:

| Aspect | FA(2) | FA(3) |
|---|---|---|
| Status | Superseded structure | Current mandatory structure |
| Schema variant in `<Naglowek>` | Declares FA(2) | Declares FA(3) |
| Core skeleton | `Naglowek` / `Podmiot1` / `Podmiot2` / `Fa` / `FaWiersz` | Same skeleton, refined fields |
| Practical effect | Rejected once FA(3) became mandatory | The version KSeF now expects |

The takeaway for anyone *reading* files: a document that still declares FA(2) in its header is
either historical or was generated by a system that hasn't been updated. A good viewer names the
variant explicitly so you notice before you rely on the file.

## Reading an FA(3) file, step by step

The workflow when a supplier or your own accounting export lands an `.xml` on your desk:

1. **Open it locally.** Drop the file into the [KSeF viewer](/file/ksef-viewer/). Parsing happens
   in the browser's XML engine, so the file never uploads.
2. **Check the parties.** Confirm `Podmiot1` (seller) and `Podmiot2` (buyer) NIPs match who you
   expect, the viewer renders them as plain names and tax IDs, not `P_n` codes.
3. **Scan the lines.** Each `FaWiersz` becomes a normal invoice row: description, quantity, unit
   price, net value, VAT rate.
4. **Reconcile the totals.** The `Fa` body's `P_15` (total due) should equal the sum of the line
   nets plus their VAT groups (`P_13_*` net / `P_14_*` VAT per rate).
5. **Save a human copy.** Print to PDF for your records, but remember the XML, not the PDF, is the
   legal instrument.

## The NIP check digit, a worked example

Every Polish NIP's tenth digit is a checksum: multiply the first nine digits by the fixed weights
6, 5, 7, 2, 3, 4, 5, 6, 7, sum the products, take the result modulo 11, and that must equal the
last digit. (If the modulo comes out as 10, the number is simply not a valid NIP.)

Take a fictional NIP `527-251-06-34`. The first nine digits are 5 2 7 2 5 1 0 6 3:

| Digit | 5 | 2 | 7 | 2 | 5 | 1 | 0 | 6 | 3 |
|---|---|---|---|---|---|---|---|---|---|
| Weight | 6 | 5 | 7 | 2 | 3 | 4 | 5 | 6 | 7 |
| Product | 30 | 10 | 49 | 4 | 15 | 4 | 0 | 36 | 21 |

The products sum to 169, and 169 mod 11 = 4, which matches the tenth digit, so the checksum holds.
Now fat-finger one digit, say the second becomes a 3, giving `537-251-06-34`. The second product
jumps from 10 to 15, the sum becomes 174, and 174 mod 11 = 9 ≠ 4. The typo fails *arithmetic*
before it ever reaches KSeF.

That is exactly the class of error worth catching on your own machine rather than as a rejected
submission. The [pre-checker](/file/ksef-validator/) runs that checksum on both parties, plus the
structural checks (required elements, date and currency formats, line items, totals consistency), clearly labelled as an **unofficial** early warning, because only KSeF's own acceptance has legal
effect.
...
KSeF turned Polish invoicing into a clearance process: FA(3) XML in, validation, KSeF number out, and only then does the invoice exist. All VAT-registered businesses have issued this way since
