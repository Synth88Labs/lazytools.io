...
`xn--…` punycode when a label mixes scripts in suspicious ways or matches known confusable patterns, so a spoofed domain often reveals itself as gibberish in the address bar. The exact rules differ between
...
Greek. So when a *single word* contains letters from two scripts, a Latin "p" next to a Cyrillic "а", something has almost certainly been substituted, because no natural language writes one word that way.
