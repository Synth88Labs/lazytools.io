...
where each entry has its own `length` and a `path` array of folder/file components. Everything else, piece length, pieces, and how the hash is computed, is identical.
