...
import { usePersistentState, exportJson, pickJson, uid, downloadSvgAsPng, downloadSvgAsPdf } from '../../lib/persist';

interface Node { id: string; parent: string | null; text: string; }
const ROOT = 'root';
const INITIAL: Node[] = [
...
const COL_W = 200, ROW_H = 46, PAD = 30, CH = 8.2;
...
export default function MindMapTool() {
  ...
  // layout (grid units) → pixels
  const pos: Record<string, { x: number; y: number }> = {};
  let leaf = 0;
  const place = (id: string, depth: number): number => {
    const cs = kids(id);
    if (!cs.length) { const y = leaf++; pos[id] = { x: depth, y }; return y; }
  ...
  place(ROOT, 0);
  const maxDepth = Math.max(...Object.values(pos).map((p) => p.x));
  const W = PAD * 2 + maxDepth * COL_W + 240;
  const H = PAD * 2 + Math.max(1, leaf) * ROW_H;
  const px = (n: Node) => ({ x: PAD + pos[n.id]!.x * COL_W, y: PAD + pos[n.id]!.y * ROW_H + ROW_H / 2 });
  ...
  function addChild(parent: string) {
    const id = uid();
    setNodes((ns) => [...ns, { id, parent, text: 'New idea' }]);
    setSel(id);
  }
  function addSibling(id: string) {
    const n = nodes.find((x) => x.id === id);
    if (!n || n.parent === null) return addChild(id);
    addChild(n.parent);
  }
  function del(id: string) {
    ...
    let grew = true;
    while (grew) { grew = false; for (const n of nodes) if (n.parent && doomed.has(n.parent) && !doomed.has(n.id)) { doomed.add(n.id); grew = true; } }
  ...
  return (
    <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm sm:p-6">
      ...
      <div class="overflow-auto rounded-xl border border-slate-200 bg-white">
        <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} width={W} height={H} xmlns="http://www.w3.org/2000/svg" class="max-w-none" style="font-family:'Plus Jakarta Sans','Segoe UI',Arial,sans-serif">
          ...
          {nodes.filter((n) => n.parent).map((n) => {
            const p = px(nodes.find((x) => x.id === n.parent)!);
            const c = px(n);
            const pw = nodeW(nodes.find((x) => x.id === n.parent)!.text);
            const x1 = p.x + pw, x2 = c.x - nodeW(n.text) / 2;
            const mid = (x1 + x2) / 2;
          ...
          {nodes.map((n) => {
            ...
            return (
              <g style="cursor:pointer" onClick={() => setSel(n.id)}>
                <rect x={bx} y={c.y - 17} width={w} height={34} rx={17}
                  fill={isRoot ? '#153056' : '#ffffff'} stroke={color} stroke-width={selected ? 3.5 : 2} />
      ...
      <p class="mt-3 text-xs text-slate-500">Click a node to select it, edit its text above, then add children or siblings. Saved in this browser; export PNG/PDF/JSON any time.</p>
    </div>
  );
}
