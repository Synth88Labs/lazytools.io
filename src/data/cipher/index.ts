...
export const CIPHER_TOOLS: CipherToolDef[] = [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What does "HELLO" become in ROT13?', a: 'URYYB. H (7) + 13 = 20 (U), E (4) + 13 = 17 (R), L (11) + 13 = 24 (Y) twice, O (14) + 13 = 27 which wraps to 1 (B). Running URYYB through ROT13 again gives back HELLO.' },
      { q: 'Is ROT13 the same as a Caesar cipher?', a: 'Yes, ROT13 is exactly a Caesar cipher with a shift of 13. What makes 13 special is that it is half of 26, so encoding and decoding are the same operation, which is not true of other Caesar shifts.' },
      { q: 'Does ROT13 change numbers or symbols?', a: 'No. Standard ROT13 rotates only the 26 letters and leaves digits, spaces and punctuation as they are. If you need numbers and symbols scrambled too, ROT47 rotates the full range of printable ASCII.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Is there a formula for the Atbash mapping?', a: 'Yes. Number the letters A = 0 through Z = 25; each letter maps to 25 minus its number. So A (0) → 25 (Z), B (1) → 24 (Y), and M (12) → 13 (N). Uppercase and lowercase are handled separately so case is preserved.' },
      { q: 'What happens to numbers and punctuation?', a: 'They pass through unchanged. Atbash only defines a mapping for the 26 letters, so digits, spaces, symbols and line breaks appear in the output exactly as you typed them.' },
      { q: 'How is Atbash different from ROT13?', a: 'Both are keyless self-inverse letter substitutions, but Atbash mirrors the alphabet (A↔Z) while ROT13 shifts it by a fixed 13 (A↔N). Atbash reverses order; ROT13 rotates position. Neither offers real security.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What does ROT47 do to the letter "A"?', a: '"A" has ASCII code 65. Applying 33 + ((65 − 33 + 47) mod 94) gives 33 + (79 mod 94) = 33 + 79 = 112, which is the character "p". So "A" becomes "p", and running "p" back through ROT47 returns "A".' },
      { q: 'Does ROT47 change spaces?', a: 'No. The space character (ASCII 32) falls just below the "!" to "~" range that ROT47 rotates, so spaces, and any control characters or newlines, are passed through unchanged, which keeps word boundaries visible in the output.' },
      { q: 'Why choose ROT47 over ROT13?', a: 'ROT13 leaves numbers and symbols readable because it only touches the 26 letters. When the text contains digits, punctuation or code where those should also be hidden, for example a config snippet or a spoiler with numbers, ROT47 rotates the whole printable-ASCII set for a more complete obfuscation.' },
