...
export const MATH_TOOLS: MathToolDef[] = [
  ...
  },
  {
    ...
    widget: 'matrix',
    how: 'Pick a size and an operation and fill in the numbers. Addition and subtraction work element by element (same size). Multiplication takes the dot product of each row of A with each column of B, so entry (i, j) of the product is row i of A dotted with column j of B. The transpose flips rows and columns. For a 2×2 matrix [[a, b], [c, d]] the determinant is simply ad − bc; for 3×3 the determinant is found by cofactor expansion, and the inverse by Gauss, Jordan elimination, reducing [A | I] until the left side is the identity, leaving A⁻¹ on the right. The 2×2 inverse has a handy closed form: swap a and d, negate b and c, and divide every entry by the determinant. A matrix with determinant zero is singular and has no inverse; the tool says so.',
    ...
    faqs: [
      ...
      { q: 'What is the determinant of a 2×2 matrix?', a: 'For [[a, b], [c, d]] it is ad − bc. So [[3, 1], [2, 4]] has determinant 3×4 − 1×2 = 10. A non-zero determinant means the matrix is invertible.' },
      { q: 'What does the determinant tell you?', a: 'Geometrically it is the factor by which the matrix scales area (2×2) or volume (3×3), and its sign shows whether orientation is flipped. A determinant of zero means the transformation collapses space to a lower dimension, which is why such a matrix cannot be inverted.' },
      { q: 'What is the identity matrix?', a: 'The square matrix with 1s on the main diagonal and 0s elsewhere. It acts like the number 1 for matrices: A × I = A, and a matrix times its inverse gives the identity, A × A⁻¹ = I.' },
    ],
