...
export const BREWING_TOOLS: BrewingToolDef[] = [
  ...
  },
  {
    ...
    how: 'Refractometers read sugar content in °Brix (percent sugar by weight), while hydrometers read specific gravity (density relative to water). The tool converts between them with the standard cubic formulas used across brewing software, in whichever direction you need — Brix to gravity or gravity to Brix. The relationship is not a simple constant multiplier: a sugar solution gets denser faster than its weight-percent rises, which is why the conversion uses a cubic polynomial rather than a straight line. A rough rule of thumb some brewers use is that gravity points are about Brix × 4 (12 °Brix ≈ 48 points, i.e. 1.048), but the full formula is more accurate across the range.',
    ...
    faqs: [
      ...
      { q: 'How do I convert specific gravity back to Brix?', a: 'The reverse cubic works from the gravity: for example 1.048 comes out to about 12 °Brix, and 1.060 to about 14.7 °Brix. Enter the gravity in the SG field and the tool returns the Brix — useful when a recipe lists one scale but your instrument reads the other.' },
      { q: 'What Brix reading do I need for a target gravity?', a: 'Roughly divide the gravity points by four as a first guess: for a 1.055 target (55 points) aim for about 13.5–14 °Brix in your pre-boil or pre-pitch wort. Use the converter for the precise figure, since the quick rule drifts at higher gravities.' },
    ],
