...
export const HOME_TOOLS: HomeToolDef[] = [
  ...
  },
  {
    ...
    how: 'The tool works out how many full drops (strips from ceiling to floor) each roll yields, given the wall height plus an allowance for trimming and pattern matching, then how many drops the room needs (perimeter ÷ roll width). Dividing one by the other, and rounding up, gives the number of rolls. A larger pattern repeat wastes more and needs more rolls. Worked through: a 10 m roll, cut into drops for a 2.4 m wall height plus a 0.1 m trim allowance, yields 10 ÷ 2.5 = 4 usable drops. A 12 m perimeter at 0.53 m roll width needs 12 ÷ 0.53 ≈ 23 drops, so 23 ÷ 4 ≈ 6 rolls. A large pattern repeat is added to each drop’s length, which cuts the drops per roll and pushes the roll count up.',
    ...
    faqs: [
      ...
      { q: 'How do I account for the pattern repeat?', a: 'Add the repeat to each drop’s cut length before working out drops per roll. If the wall is 2.4 m and the pattern repeats every 0.5 m, each drop effectively needs up to 2.9 m of paper, so a 10 m roll gives only 3 full drops instead of 4, which is why big repeats need more rolls.' },
      { q: 'How much wallpaper do I need for a feature wall?', a: 'Measure just that wall’s width, divide by the roll width for the number of drops, then divide by the drops each roll yields at your wall height. A 4 m wide, 2.4 m high wall at 0.53 m roll width needs about 8 drops, or roughly 2 rolls plus a spare.' },
    ],
...
HOME_TOOLS.push(
  ...
  },
  {
    ...
    how: 'For sod, the tool divides the lawn area by the coverage of a turf roll (about 1 m² each) and a pallet (about 50 m²), adding a little for trimming. For seed, it multiplies the area by the seeding rate (grams per m² or pounds per 1,000 sq ft) for a new lawn or overseeding. A worked example: seeding a new 150 m² lawn at 35 g/m² needs 150 × 35 = 5,250 g, about 5.25 kg of seed; overseeding the same lawn at roughly half that rate needs about 2.6 kg. For sod, a 150 m² lawn at 1 m² per roll is about 158 rolls once you add ~5% for trimming, or roughly three 50 m² pallets.',
    ...
    faqs: [
      ...
      { q: 'How much area does a bag of grass seed cover?', a: 'Divide the bag weight by the seeding rate. A 5 kg bag at a new-lawn rate of 35 g/m² covers about 5,000 ÷ 35 ≈ 143 m²; at the lighter overseeding rate it covers roughly double. Always sow at the rate on the seed label for your grass type.' },
      { q: 'When should I lay sod or sow seed?', a: 'Cool-season grasses establish best in early autumn or spring, when the soil is warm and moist but the weather is mild; warm-season grasses prefer late spring to early summer. Sod can be laid across a longer window than seed because it is already grown, but it still needs regular watering to root.' },
    ],
  ...
  },
  {
    ...
    how: 'The number of sections is the fence length divided by the post spacing (rounded up), and the number of posts is the sections plus one (an extra for the final end). The tool also estimates rails, using two or three per section depending on the fence height. Worked example: a 30 m fence at 2.4 m spacing needs ⌈30 ÷ 2.4⌉ = 13 sections, so 13 + 1 = 14 posts, plus two rails per section (26 rails) for a fence up to about 1.2 m high, or three per section (39 rails) for a taller one. The “plus one” post catches the far end of the run, which the section count alone misses.',
    ...
    faqs: [
      ...
      { q: 'How many fence panels do I need?', a: 'The number of panels equals the number of sections, the fence length divided by the panel (post-spacing) width, rounded up. A 20 m run with 1.8 m panels needs ⌈20 ÷ 1.8⌉ = 12 panels and 13 posts. Round up, because a part-width gap still needs a full or cut panel.' },
      { q: 'How much concrete do I need per fence post?', a: 'It depends on the post and hole size, but a common rule sets each post about a third of its above-ground height deep, in a hole roughly three times the post’s width. Work out the hole volume (π × radius² × depth for a round hole) and subtract the post to size the concrete, the concrete calculator handles the volume.' },
    ],
  ...
  },
  {
    ...
    how: 'The number of board rows is the deck width divided by the board width plus the gap between boards. Multiplying by the deck length gives the total linear decking, and dividing by the board length gives the number of boards. Joists are estimated from the deck length and the joist spacing. Worked example: a 4 m wide deck with 140 mm boards and a 5 mm gap needs 4,000 ÷ 145 ≈ 28 rows; across a 5 m length that is 28 × 5 = 140 m of decking, or about 47 boards if each is 3 m long, before adding ~10% for waste. Joists at 400 mm centres over a 5 m length come to ⌈5,000 ÷ 400⌉ + 1 ≈ 14 joists.',
    ...
    faqs: [
      ...
      { q: 'How do I calculate linear metres of decking?', a: 'Multiply the number of board rows by the deck length. The rows are the deck width divided by the board width plus the gap. A 3 m × 4 m deck with 145 mm effective board spacing has about 21 rows, so 21 × 4 = 84 m of decking before waste.' },
      { q: 'How many screws will I need for a deck?', a: 'For two screws at every joist crossing, multiply the number of boards by the number of joists they cross. A deck with 47 boards over 14 joists needs about 47 × 14 × 2 ≈ 1,300 screws, so buy screws by the box and keep a margin.' },
    ],
  ...
  },
  {
    ...
    how: 'A board foot is 144 cubic inches of wood, a piece 1 inch thick, 12 inches wide and 1 foot long. The formula is (thickness × width × length in inches) ÷ 144, or (thickness_in × width_in × length_ft) ÷ 12. Multiply by the number of boards, and by the price per board foot for the cost. Worked example: a piece of 8/4 (2-inch) hardwood, 8 inches wide and 6 feet (72 inches) long, is 2 × 8 × 72 ÷ 144 = 8 board feet; at $6 per board foot that board costs $48. Because board feet measure volume, doubling the thickness or the width doubles the board-foot count even though the length is unchanged.',
    ...
    faqs: [
      ...
      { q: 'What does 4/4, 8/4 mean in lumber?', a: 'Hardwood thickness is quoted in quarter-inches: 4/4 is one inch, 8/4 is two inches, 6/4 is one and a half. These are the rough (pre-planed) thicknesses used to calculate board feet, so a planed 4/4 board that finishes near 13⁄16 inch is still sold and priced as a full 4/4 board foot.' },
      { q: 'How do I estimate board feet for a project?', a: 'Add up the board feet of every piece on your cut list, then add about 20-30% for waste from defects, trimming and mistakes. Buying by total board feet with a margin is more reliable than buying an exact count of boards, since hardwood comes in random widths and lengths.' },
    ],
