...
export const GEN_TOOLS: GenToolDef[] = [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What exactly is modulo bias, and why avoid it?', a: 'The naive way to map a random byte into a range is to take the remainder after dividing by the range size. Unless the range divides the byte count evenly, some results become slightly more likely than others. Rejection sampling avoids this by discarding the few raw values that would cause the imbalance and drawing again, so every number stays exactly equally likely.' },
      { q: 'Are the numbers inclusive of the minimum and maximum?', a: 'Yes — both ends of the range are possible. Setting a range of 1 to 100 can return 1, 100, or anything between, each with equal probability. There is no off-by-one that quietly excludes an endpoint.' },
      { q: 'How do I pick a random winner from a numbered list?', a: 'Set the minimum to 1 and the maximum to the number of entries, then draw one number — that index is your winner. For several winners with no repeats, raise the count and enable unique mode so no index is drawn twice.' },
      { q: 'Can anyone verify or influence the draw?', a: 'It runs locally with OS-grade randomness — nothing external influences it, and no server logs it. For public drawings, generate on screen in front of the audience.' },
    ],
  ...
  },
  {
    ...
    widget: 'lorem',
    how: 'The generator assembles sentences from the traditional lorem ipsum vocabulary — the scrambled Latin derived from Cicero’s De Finibus Bonorum et Malorum (45 BC) that printers have used as filler since the 1500s. Words are drawn at random from that fixed vocabulary and grouped into sentences of varying length, which are in turn grouped into paragraphs; the first word of each sentence is capitalised and a full stop closes it, so the output reads with the rhythm of real prose. Sentence and paragraph lengths vary naturally so the texture resembles genuine writing, which is the entire point: realistic shape, zero meaning.',
    ...
    faqs: [
      ...
      { q: 'Can I generate an exact number of words or sentences?', a: 'Yes — switch the unit to words or sentences and enter the count, and the generator produces precisely that many. This is useful when a design spec calls for, say, a 50-word product blurb or a three-sentence caption.' },
      { q: 'Will lorem ipsum hurt my SEO if it goes live?', a: 'Live placeholder text does not help search engines understand a page and looks unfinished to visitors, so it should always be replaced before publishing. It is a design and layout aid, not real content — leaving it on a production page is a common oversight worth checking for.' },
      { q: 'Is lorem ipsum real Latin?', a: 'Not readable Latin. It is based on a genuine passage from Cicero but has been jumbled and altered over centuries of typesetting, so while fragments of real words survive ("dolor" means pain), the passage as a whole says nothing coherent by design.' },
      { q: 'Should I use lorem ipsum or real sample content?', a: 'Use lorem ipsum to review pure layout, spacing and typography without the words distracting reviewers. Switch to realistic sample content late in the process, when you need to check that actual headline lengths, names and numbers fit — real content is often longer or shorter than the placeholder and can break a layout that looked fine with lorem ipsum.' },
      { q: 'Does lorem ipsum affect how long my text looks?', a: 'Yes — its word and sentence lengths roughly mimic English prose, so a paragraph of lorem ipsum occupies about the same space real copy would. That is why it is a fair stand-in for judging line length and text density, though languages like German or Finnish run longer and are worth testing with their own sample text.' },
  ...
  },
  {
    ...
    widget: 'vcard-qr',
    how: 'Fill in the contact fields and the tool builds a standard vCard 3.0 record (BEGIN:VCARD … END:VCARD) and encodes it into the QR. Each detail becomes its own vCard line — FN and N for the name, TEL for phone, EMAIL, ORG for organisation, TITLE for job title, URL and ADR for the address — so a phone can map each field to the right slot in its contacts. Scanning the code with a phone camera opens a pre-filled “Add contact” card. It’s ideal for a business card, email signature or conference badge.',
    ...
    faqs: [
      ...
      { q: 'Does a longer vCard make the QR harder to scan?', a: 'Yes — more fields mean more data, which packs the QR with more modules and makes the pattern denser. A dense code needs to be printed larger or scanned closer to stay reliable, so include the fields you need and leave the rest blank to keep the code easy to read.' },
      { q: 'Can I include a photo in the vCard?', a: 'It is best not to. An embedded photo balloons the data far beyond what a QR code holds comfortably, producing a code too dense to scan reliably. Keep the vCard to text fields — name, phone, email, organisation, address — and let the contact app fill in a photo later.' },
      { q: 'Will the contact save correctly on both iPhone and Android?', a: 'Yes — vCard 3.0 is the version both platforms parse most reliably from a QR, and the standard field names (FN, TEL, EMAIL and so on) map to the same contact fields on each. The person scanning simply reviews the pre-filled card and taps save.' },
      { q: 'Is my contact information uploaded?', a: 'No — the QR is generated entirely in your browser, so your details are never sent to a server.' },
    ],
  ...
  },
  {
    ...
    widget: 'message-qr',
    how: 'Choose the action and the tool encodes the matching URI scheme: mailto: for a pre-filled email (with subject and body), SMSTO: for a text message, tel: for a phone number, and geo: for map coordinates. For the email action the subject and body are percent-encoded and appended as query parameters — mailto:you@example.com?subject=…&body=… — which is how the OS knows to pre-fill those fields. Scanning the code opens the phone’s email app, messaging app, dialler or maps with everything filled in, so the person scanning only has to press send or call.',
    ...
    faqs: [
      ...
      { q: 'Does scanning the code send the email or text automatically?', a: 'No — it only opens the app with the message pre-filled. The person scanning sees the recipient, subject and body (or the text and number) and must press send themselves, so nothing goes out without their action.' },
      { q: 'Where do I find latitude and longitude for a location code?', a: 'Most map apps show a place\'s coordinates when you long-press it or open its details — a pair like 40.7128, -74.0060 (latitude first, then longitude). Enter those two numbers and the geo: code points any scanner\'s map app at that exact spot.' },
      { q: 'Do these action codes expire?', a: 'No — the mailto:, SMSTO:, tel: and geo: content is encoded directly into the image with no redirect service in between, so the code keeps working indefinitely and no one tracks its scans.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What is Crockford base32 and why does ULID use it?', a: 'It is a base-32 alphabet that excludes the easily-confused letters I, L, O and U, so a ULID is safe to read aloud or type and is case-insensitive. Encoding 128 bits in base32 also takes only 26 characters, ten fewer than a UUID\'s 36, while staying human-friendly.' },
      { q: 'How far into the future can a ULID timestamp go?', a: 'The 48-bit millisecond field can represent dates until the year 10889, so it will not run out in any practical sense. Every ULID stamps the exact millisecond it was created, which is what makes the decode able to recover a real date.' },
      { q: 'Should I use ULID or UUID v7?', a: 'Both are time-sortable and serve the same purpose. UUID v7 fits systems and columns that already expect the standard 36-character UUID format, while ULID is shorter and case-insensitive. Choose ULID when compactness and readability matter, UUID v7 when UUID compatibility does.' },
      { q: 'What happens when two ULIDs are generated in the same millisecond?', a: 'They share the same 48-bit timestamp, so their ordering falls to the 80 random bits, which means IDs made in the same millisecond are not guaranteed to sort in exact creation order. The original ULID spec offers a monotonic mode that increments the random part within a millisecond to preserve order; this generator draws fresh randomness each time, which is standard and collision-safe.' },
      { q: 'Does a ULID reveal when it was created?', a: 'Yes — the first 10 characters encode the creation time to the millisecond, and anyone can decode it (the Decode tab does exactly that). That is useful for debugging and sorting, but it means a ULID is not opaque: if the creation time is sensitive, use a fully random identifier like UUID v4 or a NanoID instead.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Why is a NanoID called "URL-safe"?', a: 'Its alphabet contains only characters that need no encoding in a web address or filename: letters, digits, underscore and hyphen. That means a NanoID can drop straight into a URL path, query string or filename without being percent-encoded, unlike IDs containing "+", "/" or other reserved symbols.' },
      { q: 'How many IDs before a collision is likely?', a: 'It depends on the length. By the birthday-bound rule of thumb, a collision becomes probable only after roughly the square root of the number of possible values are generated — for the default 21-character NanoID that is astronomically large (around 2⁶³ IDs), so collisions are not a practical concern at any normal scale.' },
      { q: 'Can I shorten the alphabet or make it case-insensitive?', a: 'This tool uses the standard 64-character URL-safe alphabet. If you need case-insensitivity or a smaller symbol set, remember that shrinking the alphabet lowers the bits per character, so you must add characters to keep the same collision resistance — a trade-off worth doing deliberately.' },
      { q: 'Should I use a NanoID or a UUID for a database key?', a: 'Both work as random keys. A NanoID is shorter and URL-safe, which is nicer for public-facing IDs and links; a UUID is the format many databases, ORMs and columns expect natively. Note that neither the default NanoID nor UUID v4 is time-sortable — if you want index-friendly ordering for primary keys, use ULID or UUID v7 instead.' },
      { q: 'Is a NanoID guessable or is it a secret token?', a: 'A NanoID is unpredictable because it uses cryptographic randomness, so at the default length it is not feasible to guess a valid one. But it is an identifier, not an authorization: anyone who sees it can reuse it. Do not treat a NanoID in a URL as a password — protect resources with real access control, and use the password or passphrase generator for secrets.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How is the entropy figure calculated?', a: 'It is the word count times log₂(7776), the number of choices per word. log₂(7776) ≈ 12.9 bits, so a five-word passphrase is about 64 bits, six words about 77 bits, and seven words about 90 bits. Because each word is chosen independently and uniformly, the bits simply add up.' },
      { q: 'Does it matter that the words are common and in a public list?', a: 'No — the security comes from the number of equally-likely combinations, not from the words being secret. An attacker who knows you used the EFF list still faces 7,776 possibilities per word, which is exactly what the entropy figure counts. Secrecy of the method adds nothing; length does.' },
      { q: 'Why use words instead of random characters?', a: 'Human memory handles a short sequence of real words far better than an equally strong string of random symbols, and a six-word passphrase already out-scores a typical random password on entropy. You get strength and recall at once, which is the whole appeal of diceware.' },
      { q: 'Is the passphrase generated privately?', a: 'Yes — the word list and the generation both run in your browser; no passphrase is ever sent to a server.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What does "3d6" mean and how do I roll it?', a: 'It is dice notation for rolling three six-sided dice and adding them — common for tabletop games. Choose the d6, set the count to 3, and the tool rolls all three and shows the total. The general form NdX means "roll N dice of X sides".' },
      { q: 'Why can rolling many dice give a total near the middle more often?', a: 'Each die is uniform, but their sum is not: adding several dice makes middle totals far more likely than the extremes, because there are more combinations that produce them. Three d6 total 10 or 11 much more often than 3 or 18 — that bell-shaped spread is a property of summing dice, not a flaw in the randomness.' },
      { q: 'Is a coin flip really 50/50 here?', a: 'Yes — heads and tails are drawn from the same unbiased cryptographic source with exactly equal probability, so it is a fair 50/50 with no hidden weighting.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What do the first three octets normally mean?', a: 'In a real hardware address the first three octets are the OUI (Organizationally Unique Identifier) — a block assigned to the manufacturer — and the last three identify the specific device. A locally-administered address is self-assigned, so it does not belong to any manufacturer\'s OUI, which is exactly why it is the safe choice for test and virtual interfaces.' },
      { q: 'What is a unicast versus multicast MAC address?', a: 'The least-significant bit of the first octet flags this: 0 means unicast (one specific interface) and 1 means multicast (a group). Normal device addresses are unicast, so the generator keeps that bit clear on the first octet while it sets the separate locally-administered bit.' },
      { q: 'Why do the locally-administered addresses start with 02, 06, 0A or 0E?', a: 'Because the locally-administered bit is the second-lowest bit of the first octet. Setting it on an otherwise-even first octet produces exactly those values (…the low nibble becomes 2, 6, A or E), which is the visible signature of a self-assigned address.' },
    ],
