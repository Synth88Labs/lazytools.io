...
export const FITNESS_TOOLS: FitnessToolDef[] = [
  ...
  },
  {
    ...
    how: 'Pace (minutes per distance) and speed (distance per time) are inverses of the same thing. The tool converts your input to a common metres-per-second basis using the exact mile (1.609344 km), then expresses it as pace per km and per mile and as speed in km/h and mph. Concretely, speed in km/h = 60 ÷ (pace in min/km), and pace in min/km = 60 ÷ (speed in km/h); the per-mile figures use the 1.609344 conversion. Because everything is derived from one internal metres-per-second value, all four outputs always stay perfectly consistent with each other.',
    note: 'Treadmills usually show speed (km/h or mph) while running watches show pace (min/km or min/mile). This converter bridges the two so you can match a treadmill setting to your target pace. Note that pace is a reciprocal of speed, so equal steps in speed are not equal steps in pace: going from 10 to 11 km/h shaves more seconds off your per-km pace than going from 15 to 16 km/h.',
    faqs: [
      ...
      { q: 'How do I convert a pace to km/h by hand?', a: 'Divide 60 by your pace in minutes per km. A 5:00/km pace is 5.0 minutes, so 60 ÷ 5.0 = 12 km/h. For mph, first convert to min/mile or divide 60 by your min/mile pace.' },
      { q: 'What pace is a 5-minute mile?', a: 'A 5:00/mile pace is 12 mph, which is about 19.31 km/h, or roughly 3:07 per km. Enter 5:00 in the min/mile field to see every equivalent at once.' },
    ],
  ...
  },
  {
    ...
    how: 'Distance is simply your number of steps times your stride length. If you know your stride, enter it; otherwise the tool estimates it from your height (a walking stride is roughly 0.414 × height). Multiply and you get the distance in metres, kilometres and miles. The 0.414 factor is a widely used average ratio of stride length to standing height for walking; it is an approximation, since your true stride depends on leg length, walking speed and terrain. Once distance in metres is known, it divides by 1,000 for kilometres and by 1,609.344 for miles.',
    ...
    faqs: [
      ...
      { q: 'How many steps are in a mile?', a: 'Around 2,000 for many walkers, but it depends on stride: at a 0.75 m stride a mile (1,609 m) is about 2,145 steps, while a longer 0.85 m stride needs roughly 1,893. Enter your height or stride for a figure matched to you.' },
      { q: 'What is the difference between step length and stride length?', a: 'Step length is the distance between one foot landing and the other foot landing; stride length covers a full cycle (both feet), so it is two steps and about twice the step length. This tool uses stride length, if you measured a single step, double it before entering.' },
    ],
