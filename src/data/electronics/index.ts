...
export const ELECTRONICS_TOOLS: ElectronicsToolDef[] = [
  ...
  },
  {
    ...
    how: 'A battery\'s capacity in milliamp-hours divided by the load\'s current in milliamps gives the theoretical runtime in hours, the units cancel to leave hours because mAh ÷ mA = h. Real batteries deliver less than the label, so the tool multiplies by an efficiency factor (about 70-85%) to give a more realistic estimate alongside the theoretical maximum. Worked example: a 3,000 mAh battery driving a 250 mA load runs for 3,000 ÷ 250 = 12 hours ideally, or about 9.6 hours at 80% efficiency. If your device draw is given in watts instead of milliamps, first convert with current = power ÷ voltage.',
    ...
    faqs: [
      ...
      { q: 'How do I convert watt-hours to milliamp-hours?', a: 'Divide the energy in watt-hours by the battery voltage, then multiply by 1,000: mAh = (Wh ÷ V) × 1,000. A 18.5 Wh, 3.7 V cell is (18.5 ÷ 3.7) × 1,000 = 5,000 mAh. Compare batteries at the same voltage, since mAh alone ignores voltage.' },
      { q: 'Why can\'t I just double the capacity to double the runtime?', a: 'Roughly you can, but not exactly, larger batteries and higher discharge rates change the effective capacity, and the load itself may not be perfectly constant. Use the estimate as a guide and confirm with a real test for anything critical.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What is the reactance of a component at DC?', a: 'At 0 Hz an inductor\'s reactance is zero (it behaves like a plain wire), while a capacitor\'s reactance is infinite (it blocks steady current once charged). This is why capacitors block DC and inductors pass it.' },
      { q: 'Does reactance dissipate power like resistance?', a: 'No, an ideal reactance stores and returns energy each AC cycle rather than turning it into heat, so it draws no net (real) power. Only the resistive part of an impedance dissipates power; reactance contributes reactive power measured in VARs.' },
      { q: 'How do I combine reactance with resistance?', a: 'Use the impedance magnitude Z = √(R² + X²). For a resistor and capacitor in series with R = 40 Ω and Xc = 30 Ω, Z = √(40² + 30²) = 50 Ω. Reactance and resistance add as perpendicular components, not directly.' },
    ],
