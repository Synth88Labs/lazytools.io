...
export const MUSIC_TOOLS: MusicToolDef[] = [
  ...
  },
  {
    ...
    how: 'The metronome schedules each click ahead of time using the Web Audio clock, which is sample-accurate and doesn\'t drift the way ordinary timers do. You set the tempo (40-240 BPM) and the number of beats per bar; the first beat of each bar gets a higher-pitched accent so you can feel the downbeat, with a visual indicator that follows along. There is no formula to compute here, the value of a metronome is the steadiness of the beat, and locking each click to the audio hardware clock rather than the browser\'s general timer keeps it from stuttering when the page is busy. The beats-per-bar setting simply controls how many clicks pass before the accented downbeat returns.',
    ...
    faqs: [
      ...
      { q: 'What beats-per-bar should I set for my time signature?', a: 'Set it to the top number of the time signature, 4 for 4/4, 3 for 3/4, 6 for 6/8 (often felt in 2, so some players set 2 instead). The accent then lands on beat one of each bar, so you feel the metre rather than a flat stream of clicks.' },
      { q: 'How do I use a metronome to play a piece faster?', a: 'Practise a passage cleanly at a comfortable tempo, then raise the BPM in small steps, often about five at a time, only once you can play it accurately at the current speed. Nudging the tempo up gradually builds speed without reinforcing mistakes, which is the whole point of practising to a click.' },
      { q: 'Does the metronome keep time if I switch browser tabs?', a: 'It runs entirely in your browser, but some browsers throttle background tabs to save power, which can affect timing when the tab is hidden. Keep the metronome tab in front while you practise for the most reliable click.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How much storage do I need for a recording session?', a: 'Multiply the per-minute size by the total recorded minutes across all tracks. At 48 kHz/24-bit, one mono track is about 8.2 MB per minute, so a 24-track, 60-minute session is roughly 24 × 8.2 × 60 ≈ 11.8 GB before edits and extra takes. Record to a drive with plenty of headroom.' },
      { q: 'Why is 24-bit audio larger than 16-bit?', a: 'Because each sample stores 24 bits instead of 16-50% more data per sample, so the file is 50% larger for the same length and sample rate. The extra bits buy more dynamic range and headroom for mixing, which is why 24-bit is the production standard even though 16-bit is fine for distribution.' },
    ],
  ...
  },
  {
    ...
    how: 'BPM counts quarter-notes per minute, so a quarter-note beat is 60 ÷ BPM seconds. One bar lasts beats-per-bar × (4 ÷ denominator) × (60 ÷ BPM) seconds, the (4 ÷ denominator) term adjusts for time signatures whose beat isn\'t a quarter note. Multiply by the number of bars for a section\'s total length. Worked example: in 6/8 at 90 BPM, one bar is 6 × (4 ÷ 8) × (60 ÷ 90) = 6 × 0.5 × 0.667 = 2 seconds, so an 8-bar phrase runs 16 seconds. The (4 ÷ denominator) term is what makes a bar of 6/8 shorter than a bar of 6/4 at the same tempo, because eighth-note beats are half the length of quarter-note beats.',
    ...
    faqs: [
      ...
      { q: 'How many bars fit in a 3-minute song?', a: 'Divide the total seconds by the length of one bar. At 120 BPM in 4/4, a bar is 2 seconds, so 180 seconds holds 90 bars. Change the tempo or time signature and the tool gives the new bar length to divide by.' },
      { q: 'What tempo do I need to fit a set number of bars in a time?', a: 'Work backwards: seconds per bar = target time ÷ number of bars, then BPM = beats-per-bar × (4 ÷ denominator) × 60 ÷ seconds-per-bar. To fit 16 bars of 4/4 into 30 seconds, each bar is 1.875 s, giving 4 × 60 ÷ 1.875 = 128 BPM.' },
    ],
