...
export const PHOTOGRAPHY_TOOLS: PhotographyToolDef[] = [
  ...
  },
  {
    ...
    how: 'A time-lapse plays many still photos back as video. The number of photos is the shooting time divided by the interval between shots; the clip length is that photo count divided by the playback frame rate. The tool also estimates the memory-card space from your photo file size. The core relationship is that shooting duration and playback duration are linked only through the interval and frame rate: the effective speed-up factor equals the interval multiplied by the frame rate (a 5-second interval played at 30 fps compresses time 150×). Raising the frame rate makes motion smoother but shortens the clip for the same number of frames, so you compensate by shooting longer or with a shorter interval.',
    ...
    faqs: [
      ...
      { q: 'What is the speed-up factor of a time-lapse?', a: 'It is the interval times the playback frame rate. Shooting one frame every 10 seconds and playing back at 24 fps compresses real time by 240×, so 4 hours of real time becomes about 60 seconds of video.' },
      { q: 'Why must the shutter speed be shorter than the interval?', a: 'Each frame\'s exposure has to finish (plus a little buffer for the camera to write the file) before the next interval fires. If your night interval is 6 seconds but the exposure is 8 seconds, the intervalometer misses shots — keep the shutter comfortably shorter than the interval, or lengthen the interval.' },
    ],
  ...
  },
  {
    ...
    how: 'Print size in inches is simply the pixel count divided by the print resolution in DPI (dots per inch). The tool works out the print\'s width and height at your chosen DPI, the image\'s megapixels, and rates the quality — 300 DPI is the gold standard for photo prints, while large posters look fine at much less. Because the two dimensions scale together, print width = pixel width ÷ DPI and print height = pixel height ÷ DPI, which preserves the image\'s aspect ratio automatically. To go the other way — from a desired print size to the pixels you need — multiply the inches by the DPI: a 10 × 8 inch print at 300 DPI needs 3000 × 2400 pixels, or 7.2 megapixels.',
    ...
    faqs: [
      ...
      { q: 'How many pixels do I need for a specific print size?', a: 'Multiply each side of the print in inches by your target DPI. For an 8 × 10 inch print at 300 DPI you need 2400 × 3000 pixels (7.2 MP); the same size at 150 DPI needs only 1200 × 1500 pixels (1.8 MP).' },
      { q: 'What DPI matches a given viewing distance?', a: 'A common rule of thumb is that the resolution you need falls as viewing distance grows, because the eye resolves less detail farther away. Prints held at arm\'s length benefit from ~300 DPI; wall art viewed from a few feet is fine at 150–180 DPI; billboards seen from across a street can be 20–50 DPI.' },
    ],
