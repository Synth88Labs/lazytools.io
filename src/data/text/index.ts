...
export const TEXT_TOOLS: TextToolDef[] = [
  ...
  },
  {
    ...
    how: 'Title Case capitalizes the first letter of every word; Sentence case capitalizes after ending punctuation; the developer cases (camel, Pascal, snake, kebab, CONSTANT) first split your text on spaces, hyphens, underscores and existing capital-letter boundaries, then rejoin it in the target style. camelCase lowercases the first word and capitalizes each word after it (myVariableName); PascalCase capitalizes every word including the first (MyVariableName); alternating and inverse case flip letters by position rather than by word.',
    ...
    faqs: [
      ...
      { q: 'What is the difference between camelCase and PascalCase?', a: 'Both drop separators and capitalize word boundaries, but camelCase leaves the first letter lowercase (getUserName) while PascalCase capitalizes it (GetUserName). PascalCase suits class and type names; camelCase suits variables and functions.' },
      { q: 'How do I convert snake_case to kebab-case?', a: 'Paste the snake_case text and choose kebab-case: the converter splits on underscores and rejoins with hyphens, so first_name becomes first-name. It reads existing separators and capital boundaries, so any source case converts to any target.' },
      { q: 'When should I use CONSTANT_CASE?', a: 'For fixed values that never change at runtime — environment variables and named constants like MAX_RETRIES or API_URL. The converter splits your words and joins them with underscores in all caps, so "max retry count" becomes MAX_RETRY_COUNT.' },
      { q: 'What does alternating case actually do?', a: 'It flips letters by position rather than by word: the first letter lowercase, the next uppercase, and so on down the whole string, producing aLtErNaTiNg. It is a stylistic effect for captions and memes, not a coding or writing convention.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Does it change the lines it keeps?', a: 'No — surviving lines are returned exactly as you pasted them, including their original capitalisation and spacing. The options only affect which lines count as duplicates, not how the kept lines look.' },
      { q: 'How can I tell how many duplicates were removed?', a: 'Compare the line count before and after — the difference is the number of duplicate lines dropped. The output keeps the first occurrence of every distinct line.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How do I sort a list from Z to A?', a: 'Choose "Z → A" — it applies the same natural, case-insensitive ordering as A → Z but in reverse, so the list runs from last to first.' },
      { q: 'What is the difference between "reverse current order" and "Z → A"?', a: '"Z → A" sorts alphabetically descending; "reverse current order" simply flips your list as it stands without sorting, which is what you want to invert an already-ordered list.' },
      { q: 'Does sorting remove duplicate lines?', a: 'No — every line is kept, so identical entries end up next to each other. Run the remove-duplicate-lines tool before or after sorting if you want a unique, ordered list.' },
      { q: 'Where do blank lines end up when sorting A → Z?', a: 'Blank lines group together at the top of an ascending sort, since an empty string sorts before any text. Numbered lines then follow in natural order, so "item 2" lands before "item 10" because the digit runs compare by value.' },
  ...
  },
  {
    ...
    how: 'The default mode replaces every run of line breaks with a single space and collapses doubled spaces, turning hard-wrapped text back into a flowing paragraph. The other modes handle the narrower cases: deleting breaks outright, tidying repeated spaces and tabs, or removing only the blank lines between paragraphs. Runs of consecutive breaks are treated as one, so double-spaced text collapses without leaving extra gaps, and the join trims stray spaces at the seams where two lines meet.',
    ...
    faqs: [
      ...
      { q: 'Will it fix mixed Windows and Unix line endings?', a: 'Yes — the break-handling modes treat carriage returns and line feeds together, so text pasted from any platform is joined or cleaned the same way regardless of its original line-ending style.' },
      { q: 'How do I strip only the blank lines and keep the rest?', a: 'Choose "remove empty lines only" — it deletes lines that are empty or contain just whitespace while leaving every line with content, and its line breaks, exactly where they are.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How do I do a case-insensitive replace?', a: 'Leave "Match case" off (the default) and the tool matches regardless of capitalisation, so "Colour", "colour" and "COLOUR" are all replaced. Turn it on to replace only exact-case matches.' },
      { q: 'Are overlapping matches replaced?', a: 'No — matching is left-to-right and non-overlapping, and each stretch of text is replaced once. Your replacement is inserted literally (or via $1, $2 groups in regex mode) and is not searched again.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Why is my slug all lowercase?', a: 'URLs are case-sensitive on many servers, so mixed-case slugs risk broken links and duplicate-URL problems where /About and /about resolve differently. Lowercasing everything keeps them consistent.' },
      { q: 'What happens to numbers and existing hyphens?', a: 'Digits are kept, so "Top 10 Tips" becomes top-10-tips. Existing hyphens act as separators, and any run of spaces or symbols between words collapses to a single separator.' },
      { q: 'How long should a URL slug be?', a: 'Short enough to read at a glance — a few descriptive words is ideal. There is no hard character limit, but overly long slugs are harder to read and share, so trim filler words while keeping the terms that describe the page.' },
      { q: 'Can a slug start or end with a hyphen?', a: 'No — the generator strips separators from the start and end, so " Hello World! " becomes hello-world, never -hello-world-. Leading or trailing hyphens look broken in links and some systems reject them outright.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Does reversing characters keep my words spelled correctly?', a: 'No — character mode mirrors the whole string, so words read backwards (hello → olleh). Use word mode instead to keep every word spelled correctly while reversing their order.' },
      { q: 'How do I flip a newest-first list into oldest-first?', a: 'Choose line order — it reverses the sequence of lines without touching the text on each line, turning a top-is-newest export into chronological order.' },
      { q: 'What is the difference between character and word reversal?', a: 'Character mode mirrors the raw sequence of letters, so words read backwards (cat → tac); word mode keeps each word intact and only flips their order (one two → two one). Pick by whether you want unreadable mirror text or a re-ordered but readable sentence.' },
      { q: 'Does reversing change spacing or punctuation?', a: 'In word and line mode the words and lines are only reordered, so spelling and punctuation stay intact. Character mode mirrors everything including spaces and punctuation, so a trailing period moves to the front of the string.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How do I get one email per line for a mail-merge?', a: 'Paste the source text, choose Email addresses, and the output is already one address per line with duplicates removed — copy it straight into a spreadsheet column or mail-merge field.' },
      { q: 'Does it sort the results?', a: 'No — matches appear in the order they first occur in the text, with later duplicates dropped. Paste the output into the sort-lines tool if you need them alphabetised.' },
      { q: 'What counts as a valid URL here?', a: 'Only links that begin with http:// or https:// are matched; bare domains like example.com are skipped so file names and ordinary sentences are not mistaken for links. Each match runs until the first whitespace or trailing sentence punctuation.' },
      { q: 'How do I pull just the domains from a list of emails?', a: 'Extract the email addresses first, then use the find-and-replace tool in regex mode to delete everything up to and including the @ sign. This tool captures the full address; splitting off the domain is a quick second step.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Why does a moved paragraph show as both removed and added?', a: 'A line-level LCS diff has no concept of "moved" — it can only match lines in order, so a block that changed position appears as a deletion where it was and an insertion where it now is.' },
      { q: 'Does the order of the two texts matter?', a: 'The left text is treated as the original and the right as the revision, so additions and deletions are described relative to the left side. Swapping them swaps which lines count as added versus removed.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How do I find the most-used word?', a: 'The table is sorted by count with the most frequent word at the top, so the first row is your most-used term. Turn on "ignore common words" first to skip filler like "the" and "and".' },
      { q: 'Is punctuation counted as part of a word?', a: 'No — words are separated at punctuation and whitespace, so "text," and "text" tally together. Set a minimum word length to also drop very short tokens from the results.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Which form should I use for storing or comparing text?', a: 'NFC is the usual choice for storage and web content — it is compact and what most systems expect. Use NFD or accent-stripping when you need to compare or search while ignoring diacritics.' },
      { q: 'Does stripping accents affect non-Latin scripts?', a: 'It removes combining marks, so accented Latin letters lose their accents. Characters with no base-letter decomposition are left readable rather than emptied, so essential marks in other scripts survive.' },
      { q: 'Why do two identical-looking strings not match in my database?', a: 'One is likely stored as a composed character (é as U+00E9) and the other as a base letter plus a combining accent. Normalising both to NFC collapses them to the same code points, so they compare and de-duplicate correctly.' },
      { q: 'Is NFKC safe to use for everything?', a: 'Not always — it folds compatibility characters, so ² becomes 2 and the ﬁ ligature becomes fi, which changes meaning in maths or exact text. Use NFC to preserve appearance and reserve the NFK forms for search keys and simplification.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How do I convert a comma-separated list to tab-separated?', a: 'Set "split by" to comma and "join with" to tab — each line’s comma-separated fields are re-joined with tab characters, ready to paste straight into a spreadsheet row.' },
      { q: 'Can I transpose a single column into a row?', a: 'Yes — split by newline, enable transpose, and a one-item-per-line column becomes a single row; join with commas to get a comma-separated line.' },
      { q: 'What happens to empty fields between delimiters?', a: 'They are preserved as empty entries, so two commas in a row (a,,b) produce a blank field between a and b. This keeps columns aligned when some cells are empty rather than silently dropping them.' },
      { q: 'Can I change the delimiter without changing the layout?', a: 'Yes — set "split by" to your current delimiter and "join with" to the new one, and leave transpose off. Only the separators change; the number of lines and the order of the fields stay the same.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Will it decode emoji or symbols too?', a: 'No — it only maps styled letter and digit variants back to plain text. Ordinary emoji and symbols have no plain-letter equivalent, so they pass through untouched.' },
      { q: 'Does decoding change the meaning of my text?', a: 'No — it only replaces styled look-alike characters with the standard letters they imitate, so 𝓱𝓮𝓵𝓵𝓸 becomes hello. Spacing, real letters and punctuation are preserved.' },
      { q: 'Will it fix bold or italic text I actually formatted?', a: 'No — real bold and italics are styling applied on top of normal letters, which this tool leaves alone. It only converts stand-alone styled Unicode characters (the "fonts" from social-media generators) back to plain ASCII letters.' },
      { q: 'Why does pasted fancy text fail Ctrl-F searches?', a: 'Because each styled glyph is a different code point from the plain letter, so searching for "hello" will not match 𝓱𝓮𝓵𝓵𝓸. Decoding to plain text restores the normal letters, so search, screen readers and copy-paste all work again.' },
      { q: 'Is my text uploaded?', a: 'No — the conversion runs entirely in your browser.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Does it keep links or just the link text?', a: 'Just the visible text — an <a> tag is removed and the words it wrapped remain, but the href URL is dropped. Use the Email & URL Extractor if you specifically need the link addresses.' },
      { q: 'Can I paste a whole page’s source?', a: 'Yes — script and style blocks are stripped first, so even a full page of markup returns only its readable text, and large pages process instantly since everything runs locally.' },
      { q: 'Does it run any JavaScript in the pasted HTML?', a: 'No — script and style blocks are discarded as plain text before anything is parsed, so pasted markup is never executed. You only ever get the readable text content back, never active code.' },
      { q: 'Why are there blank lines between paragraphs in the output?', a: 'Block-level tags like paragraphs and headings become line breaks so the structure stays readable; runs of several blank lines are collapsed, but a single break between blocks is kept so paragraphs remain visually separated.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Can I start numbering from a number other than 1?', a: 'Yes — whatever you type in the value box is the first line’s number, and each line increments by one from there. Enter 0 to start at zero, or 100 to start at one hundred.' },
      { q: 'Does padding cut off long lines?', a: 'No — padding only adds spaces to reach the target width. Lines already longer than the width are left as they are, so no content is lost.' },
      { q: 'How do I turn a list into a bulleted list?', a: 'Choose "Add prefix" and enter "- " (dash space) as the value, so every line gains a leading bullet. Use "Add suffix" instead to append the same text, such as a trailing comma to build a comma-ended list.' },
      { q: 'Does numbering keep the numbers aligned?', a: 'Yes — numbers are right-aligned to the width of the largest one, so 1 through 100 line up in a tidy column instead of shifting left as the digit count grows from one to two to three places.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Does the tool change my actual words?', a: 'No — it only adds bold styling to the opening letters of each word. The text, spelling and order are identical; only the visual weight of the first few characters changes.' },
      { q: 'Where can I paste the formatted result?', a: 'Anywhere that accepts rich text — word processors, email clients and many note apps preserve the bold. Plain-text fields such as code editors and terminals strip formatting and show the letters unstyled.' },
  ...
  },
  {
    ...
    how: 'Each letter, digit and common punctuation mark is swapped for a Unicode character that resembles its 180°-rotated form, and the whole string is reversed so it reads from the bottom up. The result is normal Unicode text, so you can copy it into places that accept text — social profiles, comments, chat — where it appears flipped. Because the output is just a reordered string of look-alike glyphs, it carries no hidden data and can be copied, pasted and deleted like any other text; running the same text through again does not reliably restore the original, since a few glyphs have no exact inverse.',
    ...
    faqs: [
      ...
      { q: 'Can I flip text back to normal with this tool?', a: 'It is built to flip normal text upside down, not to reverse flipped text. A few characters have no exact inverse mapping, so round-tripping is not guaranteed — keep a copy of your original if you need it.' },
      { q: 'Should I use upside-down text in a username or password?', a: 'Usernames often accept it since it is plain Unicode, but many systems restrict which characters are allowed. Never use it in passwords — the glyphs can be stripped or normalised and lock you out.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Can I run a fair prize draw with this?', a: 'Yes — paste the entrants one per line, set the number of winners, and keep "no repeats" on so nobody wins twice. The cryptographic generator makes each entrant equally likely.' },
      { q: 'What if I ask for more picks than there are lines?', a: 'With "no repeats" on, you can\'t draw more distinct items than the list holds, so the count is capped at the list length. Turn "no repeats" off to allow the same item to be picked repeatedly.' },
