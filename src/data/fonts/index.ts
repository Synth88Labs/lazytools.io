...
export const FONT_TOOLS: FontToolDef[] = [
  ...
  },
  {
    ...
    how: 'Italics are a font style social platforms rarely expose. This tool maps your letters to Unicode "mathematical italic" characters that are slanted by design, so the tilt survives copy-and-paste into any text field. The characters come from the Mathematical Alphanumeric Symbols block (for example the letter A becomes 𝐴 at code point U+1D434), where each style, sans-serif italic and serif italic, has its own complete A, Z and a, z run. Choose the clean sans-serif italic or the more classic serif italic; digits and punctuation are left unchanged because that block defines no italic forms for them.',
    ...
    faqs: [
      ...
      { q: 'What is the difference between sans-serif and serif italic here?', a: 'Sans-serif italic (𝘢𝘣𝘤) is clean and modern with no little strokes on the letter ends; serif italic (𝑎𝑏𝑐) has those finishing strokes and reads as more formal and classic. Both are genuine Unicode characters, pick whichever suits the tone of your post.' },
      { q: 'Why don\'t the numbers in my text turn italic?', a: 'The Unicode italic block only provides italic letters, not italic digits, so numbers pass through unchanged. If you need a full italic look, keep numbers out of the styled portion.' },
      NOT_A_FONT_FAQ,
      PASTE_FAQ,
      ACCESS_FAQ,
    ],
  ...
  },
  {
    ...
    how: 'Each letter and digit is mapped to its "circled" Unicode character, the letter enclosed in a ring. These come mostly from the Enclosed Alphanumerics block (for example the letter A becomes Ⓐ at code point U+24B6, and a becomes ⓐ at U+24D0). Because they are standard Unicode symbols rather than a font effect, the bubble look copies and pastes into text fields that don\'t allow any styling, and survives being sent through chat, bios and spreadsheets.',
    ...
    faqs: [
      ...
      { q: 'Is there a filled (negative) bubble style too?', a: 'This tool produces the outlined circled letters (Ⓑⓤⓑⓑⓛⓔ). Unicode also has a separate "negative circled" set for a few capitals (like 🅐 🅑) with a solid background, but coverage there is patchy, so the outlined style is used for reliable A, Z and a, z spelling.' },
      { q: 'Why do the circles look different on each device?', a: 'Each platform draws Unicode with its own system font, so the ring may appear thicker, thinner, or as a filled disc on some phones. The underlying character is identical everywhere, only the rendering changes.' },
      NOT_A_FONT_FAQ,
      PASTE_FAQ,
      ACCESS_FAQ,
    ],
  ...
  },
  {
    ...
    how: 'This maps each character to its "full-width" Unicode form, the wide versions originally designed to sit evenly alongside Chinese, Japanese and Korean characters. The Latin letters and digits sit in the Halfwidth and Fullwidth Forms block at a fixed offset from the normal ASCII characters (for instance the letter A at U+0041 maps to Ａ at U+FF21, exactly 0xFEE0 higher), so the conversion is a simple, reliable shift. The extra built-in width gives the airy, retro "vaporwave" spacing, as pasteable plain text.',
    ...
    faqs: [
      ...
      { q: 'Is this the same as adding spaces between letters?', a: 'No. Adding spaces (l i k e   t h i s) inserts extra space characters that a reader or search can strip out, changing the text. Full-width conversion swaps each letter for a single wider character, so the spacing is built into the glyph itself rather than added between letters.' },
      { q: 'Why does the space between words get wider too?', a: 'The regular space also has a full-width counterpart (the ideographic space, U+3000), so word gaps expand in proportion with the letters, keeping the spaced-out look consistent across the whole phrase.' },
      NOT_A_FONT_FAQ,
      PASTE_FAQ,
    ],
  ...
  },
  {
    ...
    how: 'Unlike the other generators, this doesn\'t swap letters. It adds a Unicode "combining" mark after each character: the combining long stroke overlay (U+0336) for strikethrough, or the combining low line (U+0332) for underline. A combining mark is a zero-width character that a font renders on top of the preceding letter, so the tool inserts one after every character in turn. Your original letters are untouched underneath, which is why the text stays fully readable, searchable, and copies with the line attached.',
    ...
    faqs: [
      ...
      { q: 'Does the crossed-out text stay searchable and screen-readable?', a: 'Yes, because the original letters are kept and only a combining line is layered on top, the underlying word is unchanged. Search, autocorrect and screen readers still see the real text, unlike the letter-swapping fancy fonts.' },
      { q: 'Can I strike through numbers and symbols too?', a: 'Yes. The combining mark attaches to whatever character precedes it, letters, digits, punctuation and most symbols, so "$̶5̶0̶" or a crossed-out price works just as well as words.' },
      PASTE_FAQ,
    ],
  ...
  },
  {
    ...
    how: 'Two Unicode letter sets look like emoji: "regional indicators" (the A, Z symbols normally combined into flags) and "negative squared" letters (white letters on a coloured tile). This tool maps each letter of your text onto both, so you get big, blocky, emoji-style lettering as plain pasteable characters. Regional indicators are added to your base letter behind the scenes, each one is a single code point in the range U+1F1E6-U+1F1FF, while the squared letters are drawn as full emoji glyphs by the system, which is why they appear so much larger than surrounding text.',
    ...
    faqs: [
      ...
      { q: 'How do I stop letters merging into a flag?', a: 'Put a space or another character between the two letters that would form a country code, or use the squared style instead, squared letters never combine, so "US" stays as two separate tiles rather than turning into the American flag.' },
      { q: 'Do numbers and punctuation work?', a: 'No, both emoji-letter sets only define A, Z, so digits, spaces and punctuation are passed through unchanged. Spell with letters for the full effect and expect other characters to appear at their normal size.' },
      NOT_A_FONT_FAQ,
      PASTE_FAQ,
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How long will the output be?', a: 'Roughly your text length times the repeat count, plus one separator between each copy. For example the word "hi" (2 characters) repeated 5 times with a comma separator gives "hi,hi,hi,hi,hi", 10 letters plus 4 commas, so 14 characters. Numbering, if enabled, adds a few characters per line.' },
      ...
      { q: 'Does a separator go after the last copy too?', a: 'No, the separator sits between copies, not after the final one, so N copies have exactly N−1 separators. Repeating "ok" 3 times with a comma gives "ok,ok,ok", with no trailing comma. If you need a trailing separator (for example to build a list you will paste into), add it manually after copying.' },
      { q: 'Can I repeat a whole phrase or multiple lines, not just a word?', a: 'Yes, whatever you type is treated as one block and repeated intact, including spaces, punctuation and line breaks. So a full sentence or a multi-line snippet repeats exactly as entered, with your chosen separator placed between each copy.' },
    ],
