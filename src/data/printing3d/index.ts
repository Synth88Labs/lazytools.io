...
export const PRINTING3D_TOOLS: Printing3dToolDef[] = [
  ...
  },
  {
    ...
    how: 'Electricity cost is power × time × rate. The tool multiplies the printer\'s average power (in watts, ÷1000 for kilowatts) by the print time in hours to get kilowatt-hours, then multiplies by your per-kWh rate. Enter the print time in hours and minutes and your local electricity price. Concretely, kilowatt-hours = watts ÷ 1000 × hours, and cost = kWh × rate: a 120 W printer running 8 hours draws 120 ÷ 1000 × 8 = 0.96 kWh, which at $0.25/kWh is about $0.24. Because the figure scales directly with all three inputs, a slower print at the same wattage costs proportionally more, while a printer that sits idle with its heaters off between jobs adds almost nothing.',
    ...
    faqs: [
      ...
      { q: 'Does a 3D printer use much electricity on standby?', a: 'Very little — with the heaters off, a printer\'s board, display and fans draw only a few watts, so idle time between prints barely registers on the bill. Almost all the energy goes into heating the bed and nozzle while a print is actually running.' },
      { q: 'How do I lower my 3D printing electricity cost?', a: 'Print in an enclosure or a warmer room so the heated bed works less, use the lowest bed temperature the filament needs, batch several models into one job, and avoid leaving the printer heating while idle. The bed is the main draw, so anything that eases its workload saves the most.' },
    ],
