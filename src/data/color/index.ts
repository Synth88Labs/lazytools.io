...
export const COLOR_TOOLS: ColorToolDef[] = [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Worked example: what is #00ff80 in RGB?', a: '00 = 0, ff = 255, 80 = 8×16 + 0 = 128, so #00ff80 is rgb(0, 255, 128) — a bright spring green. Reading each pair independently is all it takes.' },
      { q: 'Why does two HEX digits cover exactly 0–255?', a: 'Each digit is a base-16 place, so two digits range from 00 to ff, and ff = 15×16 + 15 = 255. That is precisely the span of an 8-bit colour channel, which is why one byte per channel and one HEX pair per channel line up perfectly.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Worked example: what is rgb(255, 165, 0) in HEX?', a: '255 = ff, 165 ÷ 16 = 10 remainder 5 → a5, 0 = 00, so it is #ffa500 — the classic web orange. Each channel converts independently.' },
      { q: 'Can I shorten a HEX code to three digits?', a: 'Only when each channel\'s two digits are a repeated pair: #1133ff can shorten to #13f, but #1d87f1 cannot because 1d, 87 and f1 are not doubled digits. When in doubt keep the full six digits — every colour has a valid six-digit form.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How is a 20% shade calculated?', a: 'Multiply each channel by (1 − 0.20) = 0.8. A 20% shade of rgb(30, 135, 240) is rgb(24, 108, 192). Tints work the other direction: a 20% tint moves each channel a fifth of the way to 255.' },
      { q: 'Is a tint the same as raising HSL lightness?', a: 'Similar but not identical. This tool mixes toward white/black in sRGB, which is the classic tint/shade definition; raising HSL lightness follows a different path and can shift how saturated the color looks. For a perceptually even ladder, use the OKLCH scale generator instead.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What does the angle actually mean in CSS?', a: 'It is the compass-style direction the gradient travels, measured clockwise from up: 0deg = bottom to top, 90deg = left to right, 180deg = top to bottom, 270deg = right to left. You can also use keywords like "to right" (= 90deg) or "to bottom right".' },
      { q: 'How do I add or move colour stops?', a: 'Copy the generated CSS and add comma-separated colours, each optionally with a percentage position: linear-gradient(90deg, #1d87f1 0%, #a855f7 60%, #f43f5e 100%). Positions let you control where each colour peaks and how wide each transition is.' },
    ],
  ...
  },
  {
    ...
    widget: 'mixer',
    how: 'Mixing interpolates each RGB channel between the two colors at your chosen ratio — the same math CSS uses in color-mix(in srgb, A, B). For a ratio t (0 = all colour A, 1 = all colour B), each channel is A + (B − A) × t, computed separately for red, green and blue. Mixing #000000 and #ffffff at t = 0.5 gives (0 + 255 × 0.5) on every channel = rgb(128, 128, 128), a mid-gray. The slider sweeps the full scale, so intermediate swatches double as a mini-palette between any two brand colors.',
    ...
    faqs: [
      ...
      { q: 'How do I compute a 25/75 mix by hand?', a: 'Use A + (B − A) × t per channel with t = 0.75 for 25% A / 75% B. Mixing red rgb(255,0,0) and blue rgb(0,0,255): R = 255 + (0−255)×0.75 = 64, B = 0 + (255−0)×0.75 = 191, giving rgb(64, 0, 191) — a blue-leaning purple.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What do the L, C and H numbers mean?', a: 'L is lightness as a percentage (0% black to 100% white), C is chroma or colourfulness (0 is grey, and typical sRGB colours sit below about 0.37), and H is the hue angle in degrees (0–360). Raising C past what sRGB can show is what triggers gamut mapping.' },
      { q: 'How do I keep the same colour but make it lighter?', a: 'Raise the L value and leave C and H alone — because OKLCH lightness is perceptually even, the hue and vividness stay put while the colour brightens. That predictability is the main reason to author in OKLCH and only convert to HEX for fallback.' },
    ],
  ...
  },
  {
    ...
    how: 'Each RGB channel is linearised from sRGB (undoing the gamma curve so the numbers are proportional to light), passed through the LMS cone-response step and the exact OKLab matrices, then converted to polar form — chroma C = √(a² + b²) and hue H = atan2(b, a) — and written as oklch(L% C H). The page also shows OKLAB, CIELAB, LCH, HWB and the HEX form of the same color, so you can copy whichever your CSS needs.',
    ...
    faqs: [
      ...
      { q: 'Why linearise the RGB channels first?', a: 'sRGB values are gamma-encoded, so they are not directly proportional to light intensity. Linearising undoes that curve, which is required before the OKLab matrices — which assume linear light — can be applied. Skipping it would give wrong lightness and hue.' },
      { q: 'Is every RGB colour representable in OKLCH?', a: 'Yes — every sRGB (and therefore every RGB or HEX) colour has an exact OKLCH value, so RGB to OKLCH is lossless. Gamut trouble only appears in the reverse direction, when an OKLCH colour is too vivid for an sRGB screen to display.' },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'What is the difference between deuteranopia and deuteranomaly?', a: 'Deuteranopia is the dichromatic form, where the green-sensitive (M) cones are effectively absent. Deuteranomaly is the more common anomalous-trichromat form, where those cones are present but shifted, giving reduced green sensitivity. The severity slider models the range between them.' },
      { q: 'Why test deuteranopia specifically?', a: 'Because it is the single most common colour-vision deficiency, a design that stays clear under deuteranopia usually holds up for most colour-blind users. Red/green pairings — pass/fail states, chart series, status dots — are where it bites hardest, so those are the first things to check.' },
      { q: 'What colours are hardest to tell apart under deuteranopia?', a: 'Red versus green is the classic confusion, but it also blurs green versus brown, green versus orange, and some blues versus purples (since purple is red plus blue, and the red part fades). If a chart or map relies on any of those pairs to carry meaning, they can collapse together — the simulator shows exactly which ones.' },
      { q: 'How do I fix a design that fails under deuteranopia?', a: 'Do not rely on hue alone. Add a second cue — text labels, icons, patterns or a clear lightness difference — so meaning survives without colour. For chart series, differ the lightness of adjacent lines and use direct labels; for status, pair a red/green colour with a distinct shape (cross versus tick).' },
    ],
  ...
  },
  {
    ...
    widget: 'cvd',
    cvdType: 'protanopia',
    how: 'Each pixel or swatch is linearised and multiplied by the published Machado (2009) protanopia matrix, then converted back to sRGB, with a severity slider for milder protanomaly. The transform models the loss of the red-sensitive (L) cone, so red light contributes far less perceived brightness — reds darken and slide toward the greens and browns they get confused with. Severity at 100% is the full dichromat matrix; lower settings blend original and simulated pixels to approximate protanomaly. Images are processed on a canvas and never uploaded.',
    ...
    faqs: [
      ...
      { q: 'How common is protanopia?', a: 'Red-weakness in all its forms (protanopia plus protanomaly) affects around 1–2% of men, less than deuteranopia but still worth testing — especially because it dims reds so strongly. It is very rare in women.' },
      { q: 'What is the practical thing to check under protanopia?', a: 'Anything where red carries meaning against a dark or green backdrop: red error text, red alert badges, red-vs-green chart lines. Because reds lose brightness, they can drop below usable contrast — pair red with a shape, icon, or lighter tint so the meaning survives.' },
      { q: 'Why does protanopia darken reds so much?', a: 'The red-sensitive (L) cone contributes most of the perceived brightness of red light. With it absent, red stimulates the remaining cones only weakly, so a pure red looks dim and muddy rather than vivid. That is why red-on-black text — which relies on the red being bright — can nearly vanish for someone with protanopia, a problem deuteranopia does not share as strongly.' },
      { q: 'How do I make reds work for protanopia?', a: 'Shift important reds toward orange or add lightness so they do not go dim, and never depend on a red/green distinction alone. Back up colour with icons, labels or shapes, and check red text against its background in the contrast checker, since the effective brightness of red is what drops under protanopia.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'How is tritanopia different from red-green color blindness?', a: 'It affects the blue-yellow axis rather than red-green, and it is inherited differently — it does not favour men the way protan and deutan types do, so it affects both sexes about equally. It is also far rarer.' },
      { q: 'What should I look at under tritanopia?', a: 'Blue-versus-teal and yellow-versus-pink cues are the danger zones, since those hues can collapse together. Common culprits are blue links next to teal accents and yellow highlights against light-green or pink states — separate them by lightness or add a non-colour cue.' },
      { q: 'Why is tritanopia inherited differently from red-green types?', a: 'The blue (S) cone gene sits on chromosome 7, not the X chromosome, so tritanopia is not sex-linked and affects men and women about equally. The red and green cone genes are on the X chromosome, which is why protanopia and deuteranopia are far more common in men. Tritanopia is also rarer overall, well under 1% of people.' },
      { q: 'Should I still test tritanopia if it is so rare?', a: 'Yes — it is quick and it catches blue/yellow ambiguities that also trouble more common conditions, ageing eyes (the lens yellows with age, dulling blues), and low-light viewing. If your interface leans on blue and yellow for links, highlights or status, a tritanopia pass is worthwhile insurance even though the condition itself is uncommon.' },
    ],
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: 'Worked example: what is the complement of a hue-120 green?', a: 'Add 180° to hue 120° to get hue 300° — a magenta — keeping the same saturation and lightness. That opposite pairing is why green and magenta read as maximally contrasting.' },
      ...
      { q: 'Is a complementary color the same as its inverse (255 minus each channel)?', a: 'No — those are two different operations. A color-wheel complement rotates only the hue 180° and keeps saturation and lightness, so a dark blue stays dark. An RGB inverse (255−R, 255−G, 255−B) also flips lightness, so a dark blue becomes a light orange. This tool gives the true hue complement.' },
      { q: 'How do I use complementary colors accessibly?', a: 'Two complementary hues at the same lightness can clash and fail contrast for text, and red/green complements are hard for color-blind users. Pair them by making one much lighter or darker than the other, and check the pairing in the contrast checker before using it for text on a background.' },
  ...
  },
  {
    ...
    how: 'A triadic scheme takes three hues spaced 120° apart on the wheel (base, base+120°, base+240°), keeping saturation and lightness. Because 3 × 120° = 360°, the three hues divide the wheel into equal thirds — an equilateral triangle inscribed in the color circle — which is what makes the set feel balanced rather than lopsided. Angles that exceed 360° wrap around (base 300° + 120° = 60°). The tool computes the two partner hues exactly and shows all three with HEX, HSL and OKLCH.',
    ...
    faqs: [
      ...
      { q: 'Worked example: the triad of a hue-0 red?', a: 'From hue 0° (red), adding 120° gives hue 120° (green) and adding 240° gives hue 240° (blue) — the classic red/green/blue triad — all at the base saturation and lightness. Shift the base hue and the whole triangle rotates with it.' },
      ...
      { q: 'What is the difference between a triadic and a tetradic scheme?', a: 'A triad uses three hues 120° apart (an equilateral triangle on the wheel); a tetrad uses four. Because a triad has one fewer color, it is easier to balance and less likely to feel busy. Reach for a tetrad when you need two complementary pairs; stay with a triad for a simpler, punchy palette.' },
      { q: 'Which of the three triad colors should be the accent?', a: 'A common rule is the 60-30-10 split: about 60% of the design in one dominant color, 30% in the second, and 10% in the third as the accent — often the most saturated of the three. Using all three at equal strength is what makes a triad look garish, so let one lead.' },
  ...
  },
  {
    ...
    how: 'An analogous scheme uses hues adjacent on the wheel — typically base−30°, base, base+30° (and optionally ±60°) — at the same saturation and lightness. Concretely, only the hue angle H changes: a base of hsl(210, 80%, 50%) yields partners at hue 180° and 240°, while S and L stay fixed, so the three colors share the same vividness and brightness and differ only in hue. Because hue is circular, the tool wraps angles past 360° back into 0–360° (base 350° + 30° = 20°). It computes the neighbours exactly and shows them with HEX, HSL and OKLCH.',
    ...
    faqs: [
      ...
      { q: 'Worked example: what are the analogous hues of a hue-210 blue?', a: 'Stepping ±30° gives hue 180° (cyan) and hue 240° (indigo), keeping the same saturation and lightness. Widening to ±60° adds hue 150° and 270° for a five-color set. The base stays the visual anchor.' },
      { q: 'Should I space by 30° or something else?', a: 'Thirty degrees is the common default, but any small, equal step works — 20° gives a tighter, more monochromatic feel; 45° pushes toward contrast. Keeping the step equal on both sides is what preserves the balanced, harmonious look.' },
  ...
  },
  {
    ...
    how: 'Delta-E (ΔE) is the distance between two colours in the perceptually-uniform CIELAB space, where each colour is described by L (lightness), a (green–red axis) and b (blue–yellow axis). The tool converts each colour from RGB through XYZ to Lab, then computes the difference two ways: the modern, perceptually-accurate CIEDE2000 formula, and the older CIE76 (a simple straight-line distance) for comparison. CIE76 is just √(ΔL² + Δa² + Δb²); CIEDE2000 keeps that skeleton but reweights the lightness, chroma and hue terms and adds a rotation correction so the number tracks human perception more faithfully. A smaller ΔE means a closer match.',
    ...
    faqs: [
      ...
      { q: 'What does a ΔE of 5 mean in practice?', a: 'A clearly visible difference — well past the just-noticeable threshold of about 1. At ΔE 5 most people would call the two colours a mismatch rather than a close match; brand and print tolerances usually reject anything above 2–3.' },
      { q: 'Why can CIE76 and CIEDE2000 disagree?', a: 'CIE76 treats every direction in Lab equally, but our eyes are more forgiving of chroma differences in saturated colours and more sensitive to hue shifts. CIEDE2000 corrects for that, so two colours with a moderate CIE76 distance — often in blues or near-neutrals — can score a smaller, more realistic CIEDE2000. Trust CIEDE2000 when they differ.' },
    ],
