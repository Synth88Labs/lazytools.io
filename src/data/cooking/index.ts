...
export const COOKING_TOOLS: CookingToolDef[] = [
  ...
  },
  {
    ...
    how: 'The tool finds a single scaling factor — the new servings divided by the original (or the multiplier you type) — and multiplies every ingredient quantity by it. Quantities can be decimals or fractions (enter ½ as 1/2 and 1½ as 1 1/2), and the scaled results appear alongside each ingredient. For example, taking a recipe from 4 servings to 6 gives a factor of 6 ÷ 4 = 1.5, so 2 cups of flour becomes 3 cups and 150 g of sugar becomes 225 g. Because one factor is applied to everything, the proportions between ingredients stay identical — which is exactly what keeps the scaled dish tasting like the original.',
    ...
    faqs: [
      ...
      { q: 'How do I scale a recipe by weight instead of servings?', a: 'Divide your target total weight by the recipe\'s original total to get the factor, then apply it to every ingredient. If a dough recipe makes 900 g and you need 1,200 g, the factor is 1,200 ÷ 900 ≈ 1.33 — the same single-factor method the tool uses whether you drive it by servings or by a multiplier.' },
      { q: 'Why do salt and spices need tasting when I scale up a lot?', a: 'Seasoning isn\'t always perceived in strict proportion to volume, and a large batch has a smaller surface-area-to-volume ratio, so strong flavours can build up. Multiply them as a starting point, then taste and adjust — it\'s easier to add more than to fix an over-salted big batch.' },
    ],
  ...
  },
  {
    ...
    how: "In baker's percentage, the flour is always 100%, and every other ingredient is expressed as a percentage of the flour weight: an ingredient’s weight ÷ flour weight × 100. Hydration is the total liquid (water, milk) as a percentage of flour. This lets you scale a formula to any batch size and compare recipes on equal terms. Worked through: with 500 g flour and 325 g water, hydration is 325 ÷ 500 × 100 = 65%; 10 g of salt is 10 ÷ 500 × 100 = 2%. When a recipe uses more than one flour, the total flour is still the 100% baseline, and each flour is also given as its own share of that total.",
    ...
    faqs: [
      ...
      { q: "How do I include the water in a sourdough starter or preferment?", a: 'For accurate hydration, split your starter or preferment into its flour and water parts and add each to the totals. A 100 g starter at 100% hydration is 50 g flour and 50 g water, so those join the flour total and the water total before you work out the overall percentages — otherwise the true hydration reads lower than it is.' },
      { q: "What is a typical salt percentage in bread?", a: 'Around 1.8–2.2% of the flour weight, with 2% the common default — enough to season the dough and control fermentation without tasting salty. Below about 1% bread tastes flat; much above 2.5% it starts to slow the yeast noticeably. Expressing salt as a percentage is what keeps it correct at any batch size.' },
    ],
  ...
  },
  {
    ...
    how: 'Good coffee is about proportion. The tool uses a ratio of 1 gram of coffee to N grams of water: enter the ratio and one quantity, and it computes the other by multiplying or dividing. Water is measured by weight, which is why it’s given in grams (1 g of water ≈ 1 mL). So at a 1:16 ratio, 500 g of water needs 500 ÷ 16 ≈ 31 g of coffee; conversely 30 g of coffee calls for 30 × 16 = 480 g of water. Note the ratio counts the water that ends up in the cup — a little is always lost to the wet grounds, so brewing a touch more water than your target cup size is normal.',
    ...
    faqs: [
      ...
      { q: 'How do I make my coffee stronger or weaker without changing the amount?', a: 'Change the ratio, not the cup size. A lower number (more coffee per unit water), say 1:14, brews stronger; a higher number like 1:18 brews lighter. Keep the water fixed and let the tool recalculate the coffee — that isolates strength from how much you\'re making.' },
      { q: 'Is coffee-to-water ratio the same as strength or extraction?', a: 'Not quite. The ratio sets the brew\'s strength (how concentrated it is), while extraction — how much flavour is pulled from the grounds — is governed mainly by grind size, water temperature and brew time. You can hit the same ratio yet get sour or bitter coffee if the extraction is off, so treat the ratio as one of several dials.' },
    ],
