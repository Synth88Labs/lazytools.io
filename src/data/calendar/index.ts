...
export const CALENDAR_TOOLS: CalendarToolDef[] = [
  ...
  },
  {
    ...
    widget: 'single',
    system: 'hebrew',
    how: 'The Hebrew calendar is lunisolar: months follow the moon (29 or 30 days) but an extra month (Adar I) is inserted in 7 of every 19 years to keep the festivals aligned with the seasons — so a common year has 12 months and a leap year 13. This converter expresses any Gregorian date in the Hebrew calendar using your browser\'s ICU data. Years are counted AM (Anno Mundi) from the traditional date of creation, currently around 5786. The 19-year cycle (called the Metonic cycle) works because 19 solar years almost exactly equal 235 lunar months — 12 ordinary years of 12 months plus 7 leap years of 13 — which is what keeps Passover in spring century after century. Year length itself varies between 353 and 385 days depending on whether it is a leap year and on two small calendrical adjustments.',
    ...
    faqs: [
      ...
      { q: 'Which month is added in a leap year?', a: 'A second month of Adar. The extra month, Adar I (also called Adar Aleph), is inserted before the usual Adar, which then becomes Adar II — so a leap year has 13 months. Purim and other Adar observances fall in Adar II in a leap year, keeping them roughly a month before Passover.' },
      { q: 'Why do the Hebrew and Gregorian years not line up?', a: 'The Hebrew year begins at Rosh Hashanah in the autumn (September or October), not on 1 January, so a single Hebrew year straddles two Gregorian ones. That is why a given Gregorian date can map to one of two Hebrew year numbers depending on whether it falls before or after Rosh Hashanah.' },
