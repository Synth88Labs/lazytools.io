...
/**
 * US Navy body-fat estimate (%) from tape measurements in INCHES and log₁₀.
 * Men:   86.010·log₁₀(waist − neck) − 70.041·log₁₀(height) + 36.76
 * Women: 163.205·log₁₀(waist + hip − neck) − 97.684·log₁₀(height) − 78.387
 * An estimate only — accuracy is roughly ±3–4% vs. a DEXA scan.
 */
export function navyBodyFat(sex: Sex, heightIn: number, neckIn: number, waistIn: number, hipIn = 0): number | null {
...
/** Body-fat category thresholds (ACE, %) — informational bands. */
export function bodyFatCategory(sex: Sex, pct: number): string {
  const bands: [number, string][] = sex === 'male'
    ? [[6, 'Essential fat'], [14, 'Athletes'], [18, 'Fitness'], [25, 'Average'], [Infinity, 'Above average']]
    : [[14, 'Essential fat'], [21, 'Athletes'], [25, 'Fitness'], [32, 'Average'], [Infinity, 'Above average']];
...
/**
 * Basal metabolic rate (kcal/day) by the Mifflin-St Jeor equation:
 * 10·kg + 6.25·cm − 5·age + (5 for men, −161 for women). The most accurate of
 * the common predictive BMR formulas for the general population.
 */
...
/** Standard activity multipliers for total daily energy expenditure. */
export const ACTIVITY_FACTORS: { id: string; label: string; factor: number }[] = [
  ...
  { id: 'extra', label: 'Extra active (hard job / 2×/day)', factor: 1.9 },
];

/** Total daily energy expenditure = BMR × activity factor. */
export function tdee(bmr: number, factor: number): number { return bmr * factor; }
