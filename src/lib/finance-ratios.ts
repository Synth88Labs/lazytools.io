  ...
  if (totalEquity > 0) {
    ...
    out.push(R('roe', 'Return on equity (ROE)', roe, pct(roe),
      ...
      roe < 0 ? `Shareholders are losing ${pct(-roe)} on their equity.`
        : `Shareholders earn ${pct(roe)} on their equity. Often 15 to 20% is considered strong, but a high ROE can be driven by heavy debt, so read it alongside the leverage ratios.`));
  }
  return out;
}
