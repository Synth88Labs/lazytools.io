...
const HUBS = {
  ...
  solar: { tools: SOLAR_TOOLS, blurb: 'Solar & off-grid energy calculators computed in-browser, no location access / nothing uploaded; industry-standard formulas + reference values verified vs NREL PVWatts & battery-maker refs (cited, site-specific figures user-entered): solar panel output (daily kWh = watts × peak-sun-hours × derate ÷ 1000; use worst-month PSH not annual avg; derate ~0.77–0.86, PVWatts 14% DC loss); off-grid load (Σ appliance W × qty × hours/day → daily Wh, editable table + presets); battery bank (Ah = daily-Wh × autonomy-days ÷ DoD ÷ system-V; DoD 50% lead-acid / 80% LiFePO4; cold derates lead-acid & lithium must not charge <0°C); inverter size (continuous = running-W × 1.25 headroom; surge covers largest motor ~2–3× running); appliance energy cost (W × h × rate → day/month/year); solar payback (net-cost ÷ (annual-kWh × rate); simple, ignores inflation/degradation — not financial advice); DC voltage drop (Vdrop = 2·L·I·ρ ÷ A, copper ρ=1.68e-8; ≤3% NEC / ≤2% solar best-practice; AWG selector); battery charge time (Ah ÷ A × ~1.15 losses; C-rate = A ÷ Ah, lead-acid 0.1–0.3C / LiFePO4 0.5–1C). Estimates for planning, not a substitute for a site survey or electrical code.' },
  '3d-printing': { tools: PRINTING3D_TOOLS, blurb: ' 3D printing calculators computed in-browser, no upload / no model files leave the device: filament weight↔length↔volume (filament is a cylinder: mass = π·(d/2)²·length·density; PLA 1.24, PETG 1.27, ABS 1.04 g/cm³ etc — a 1kg PLA 1.75mm spool ≈ 335m; densities cited, brand-varying for Nylon/wood/CF flagged); filament cost (grams÷1000×price/kg); print electricity cost (W×h×rate, typical FDM 50–150W, bed dominates); model scale (linear factor + VOLUME scales by the CUBE — 200% = 8× filament/time; + fit-to-bed helper); E-steps calibration (new = current×(requested÷actual extruded), 100mm test, M92/M500); flow / extrusion multiplier (new% = current×(target÷measured wall), classic single-wall method, calibrate E-steps first); volumetric flow (mm³/s = layer×width×speed vs hotend max ~10–15 standard / 20–40 high-flow — the real speed limit); resin cost (mL×price/L + waste, density ~1.10 g/mL). Exact geometry + standard maker calibration formulas; reference densities/wattage cited (Simplify3D, Prusa, Formlabs, Ellis Print Tuning Guide) and caveated as varying.' },
