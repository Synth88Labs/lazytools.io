...
const slugs = [
  ...
  ...FITNESS_TOOLS.map((t) => `fitness/${t.slug}`),
  ...PET_TOOLS.map((t) => `pets/${t.slug}`),
  ...GARDEN_TOOLS.map((t) => `garden/${t.slug}`),
  ...
  ...SOLAR_TOOLS.map((t) => `solar/${t.slug}`),
  ...BREWING_TOOLS.map((t) => `brewing/${t.slug}`),
].sort();
