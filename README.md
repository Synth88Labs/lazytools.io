...
| Category | Highlights |
|---|---|
| [Unit Converters](https://lazytools.io/units/) | **20 quantities** — length, weight, temperature, volume, area, speed, time, data storage, pressure, energy, power, **data-transfer-rate** (Mbps↔MB/s), **frequency** (Hz/RPM/rad·s), **torque** (N·m↔lbf·ft), **force** (N/lbf/kgf), **density**, **volumetric flow-rate** (L/min↔GPM), **angle** (deg/rad/gon/arcmin/arcsec), **acceleration** (m/s²/g/ft-s²), **illuminance** (lux/foot-candle) — exact NIST/BIPM factors (audited by verify-factors.ts), per-pair editorial notes |
...
| [Mathematics](https://lazytools.io/math/) | **32 tools, exact arithmetic** — fractions with steps, primes (Miller–Rabin), quadratics with radical roots, statistics, Roman numerals, nCr/nPr at BigInt scale, **triangle solver** (law of sines/cosines, ambiguous SSA), **matrix calculator** (det/inverse/multiply), **logarithm** (any base + antilog) |
