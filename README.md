...
| Category | Highlights |
|---|---|
| [Unit Converters](https://lazytools.io/units/) | **27 quantities** — length, weight, temperature, volume, area, speed, time, data storage, pressure, energy, power, **data-transfer-rate** (Mbps↔MB/s), **frequency** (Hz/RPM/rad·s), **torque** (N·m↔lbf·ft), **force** (N/lbf/kgf), **density**, **volumetric flow-rate** (L/min↔GPM), **angle** (deg/rad/gon/arcmin/arcsec), **acceleration** (m/s²/g/ft-s²), **illuminance** (lux/foot-candle), **magnetic field** (tesla/gauss), **dynamic viscosity** (Pa·s/poise/cP), **kinematic viscosity** (m²/s/stokes/cSt), **radioactivity** (becquerel/curie), **luminance** (nit/cd·m²/foot-lambert), **electric charge** (coulomb/Ah/mAh), **typography** (point/pixel/pica, 96px=72pt) — exact NIST/BIPM factors (audited by verify-factors.ts), per-pair editorial notes |
| [Calculators](https://lazytools.io/calc/) | **29 tools** — percentage, EMI, BMI, age, tip, discount, interest, mortgage, sales tax, fuel cost, GPA, **test grade (EZ grader)**, **unit price**, **overtime pay**, **square footage**, **cost per square foot**, **dilution ratio (1:N mixing)**… |
...
| [Mathematics](https://lazytools.io/math/) | **31 tools, exact arithmetic** — fractions with steps, primes (Miller–Rabin), quadratics with radical roots, statistics, Roman numerals, nCr/nPr at BigInt scale, **triangle solver** (law of sines/cosines, ambiguous SSA), **matrix calculator** (det/inverse/multiply), **logarithm** (any base + antilog) |
...
| [Homebrewing](https://lazytools.io/brewing/) | **12 tools** — **ABV** ((OG−FG)×131.25 + Hall's accurate formula, attenuation, calories), **IBU** (Tinseth model, editable hop-additions table), **priming sugar** (CO₂ stoichiometry, bottle carbonation), **Brix↔SG**, **hydrometer temperature correction**, **strike water** (Palmer, metric/imperial), **gravity dilution**, **refractometer FG** (Sean Terrill cubic + WCF) — established brewing formulas verified vs Palmer/Tinseth/Terrill, Node-tested |
...
| [Pets & Animals](https://lazytools.io/pets/) | **11 tools** — **dog age** (dog↔human years via the 2020 epigenetic formula 16×ln(age)+31 + size-based chart — busts the ×7 myth), **cat age** (15/+9/+4 chart + life stage), **dog & cat food** portions (RER 70×kg^0.75 × MER life-stage factor → cups), **pet gestation** due dates (13 species), **aquarium volume** (L/US-gal/UK-gal), **pet water intake** (~50–60 ml/kg), **dog crate size** (AKC +2–4 in) — vet-sourced formulas, Node-tested |
...
| [Charts & Design](https://lazytools.io/charts/) | **6 chart makers** (10 self-hosted fonts embedded into exports, custom text/series/background colors, transparent bg) — **bar**, **line**, **pie & donut**, **funnel** (conversion stages + %), **radar/spider** (multi-axis profile), **waterfall** (running total, +/- floating bars) — paste “label, value” lines, pick a palette, download a crisp 2× **PNG** or an editable **SVG**; drawn with SVG in your browser, nothing uploaded |
| [Fonts & Typography](https://lazytools.io/fonts/) | **24 tools** — **fancy font & text generator** (21 Unicode styles: 𝗯𝗼𝗹𝗱, 𝘪𝘵𝘢𝘭𝘪𝘤, 𝓼𝓬𝓻𝓲𝓹𝓽, 𝔣𝔯𝔞𝔨𝔱𝔲𝔯, 𝕕𝕠𝕦𝕓𝕝𝕖-𝕤𝕥𝕣𝕦𝕔𝕜, ᴍᴀʟʟ ᴄᴀᴘꜱ, ⓑⓤⓑⓑⓛⓔ, ｗｉｄｅ, s̶t̶r̶i̶k̶e̶ — real Unicode you can paste into Instagram/TikTok/X bios, gaps patched from Letterlike Symbols so no blank boxes), plus focused **bold**, **italic**, **cursive**, **small-caps**, **bubble**, **wide**, **strikethrough**, **superscript/subscript** & **emoji-letter** generators — a **glitch/Zalgo (cursed text)** generator with a madness slider, an **invisible / blank name** generator (Free Fire, WhatsApp, Discord), a **symbols & special-characters** copy-paste picker (arrows, stars, hearts, math…), a **Discord colored-text (ANSI)** generator, a **text repeater**, a **mirror / backwards text** generator, a **Lenny face & kaomoji** picker — and for CSS/design: **px↔rem converter**, **modular type-scale generator** (golden ratio + 7 more → copy-paste custom properties), **line-height calculator** (comfortable-reading bands), **clamp() fluid-typography generator**, **gradient-text (background-clip) generator** with PNG export, and a **line-length / measure calculator** (45–75 CPL), and an **ASCII-art / big-text generator** (self-contained FIGlet renderer, 10 classic fonts — Standard, Big, Slant, ANSI Shadow… — with kerning, copy & .txt download) — all client-side, nothing uploaded |
...
| [File & Data](https://lazytools.io/file/) | CSV/JSON/XML/YAML, **TOML↔JSON**, **XML formatter/minifier**, Markdown↔HTML (**HTML→Markdown** via Turndown), **Excel↔data** (xlsx→CSV, xlsx→JSON, CSV→xlsx via SheetJS), **e-invoicing viewers: Factur-X (FR), KSeF FA(3) (PL), Peppol BIS (BE), XRechnung/ZUGFeRD (DE)** |
| [Developer Tools](https://lazytools.io/dev/) | Base64, **Base32** (RFC 4648), hashes, **HMAC** (SHA-1/256/384/512, keyed), JWT decoder + **JWT encoder/signer** (HS256/384/512), regex, number-base, **query-string parser** (↔ JSON), **HTTP status code lookup**, **LLM token counter (exact o200k)**, **IBAN validator** (ISO 13616 mod-97), **ISBN-10↔13 converter**, **Punycode/IDN converter** (xn--), **JSON diff** (semantic, key-aware), **Ethereum unit converter** (BigInt-exact wei⇄ether), **Keccak-256/SHA-3 + function selector**, **EIP-55 address checksum**, **JSON string escaper/unescaper**, **Unicode escape converter** (\uXXXX ↔ text), **text ↔ hex converter** (UTF-8) |
...
| [Privacy & Security](https://lazytools.io/security/) | EXIF remover, AES-256 file encryption, file hash, **TOTP/2FA code generator** (RFC 6238, live countdown — secret never uploaded), **bcrypt hash generator & verifier** (tunable cost, salted), **PII redactor** (mask emails/phones/SSNs/cards[Luhn]/IPs/IBANs in text before pasting into AI — 100% client-side) |
| [Image Tools](https://lazytools.io/image/) | **18 tools** — compress, convert, resize, **resize to an exact KB target** (passport/exam upload limits — binary-search JPEG/WebP quality + downscale), **crop** (rectangular + aspect presets), **flip/mirror**, **favicon generator** (one image → full PNG set + multi-size favicon.ico + manifest + HTML snippet, zipped), **watermark** (text, opacity, tiled diagonal), **HEIC→JPG** (libheif wasm), image→Base64, **rotate** (90/180/270 + EXIF-orientation fix), **circle crop** (transparent-PNG avatars), **split image**, **EXIF metadata viewer**, **annotate**, **SVG→PNG** (rasterise at any size), **Base64→image** (decode data URLs), **combine images** (stitch vertical/horizontal) |
...
| [Audio & Video](https://lazytools.io/video/) | **9 tools** — trim, speed, volume, WAV convert (Web Audio), audio merger, **video frame extractor** (scrub + capture a still at native resolution via `<video>` + canvas — no ffmpeg.wasm, so no COOP/COEP headers and no 25 MB download), **subtitles: SRT→VTT, VTT→SRT, subtitle shifter/resync** (pure-JS cue parsing) |

Plus **121 in-depth guides** on the [blog](https://lazytools.io/blog/) — each with custom infographics,
