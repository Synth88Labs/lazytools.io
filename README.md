...
| Category | Highlights |
|---|---|
| [Unit Converters](https://lazytools.io/units/) | **21 quantities** — length, weight, temperature, volume, area, speed, time, data storage, pressure, energy, power, **data-transfer-rate** (Mbps↔MB/s), **frequency** (Hz/RPM/rad·s), **torque** (N·m↔lbf·ft), **force** (N/lbf/kgf), **density**, **volumetric flow-rate** (L/min↔GPM), **angle** (deg/rad/gon/arcmin/arcsec), **acceleration** (m/s²/g/ft-s²), **illuminance** (lux/foot-candle), **magnetic field** (tesla/gauss) — exact NIST/BIPM factors (audited by verify-factors.ts), per-pair editorial notes |
