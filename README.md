...
| [Developer Tools](https://lazytools.io/dev/) | Base64, **Base32** (RFC 4648), hashes, JWT, regex, number-base, **query-string parser** (↔ JSON), **HTTP status code lookup**, **LLM token counter (exact o200k)**, **IBAN validator** (ISO 13616 mod-97), **ISBN-10↔13 converter**, **Punycode/IDN converter** (xn--), **JSON diff** (semantic, key-aware), **Ethereum unit converter** (BigInt-exact wei⇄ether), **Keccak-256/SHA-3 + function selector**, **EIP-55 address checksum**, **JSON string escaper/unescaper**, **Unicode escape converter** (\uXXXX ↔ text), **text ↔ hex converter** (UTF-8) |
...
| [Privacy & Security](https://lazytools.io/security/) | EXIF remover, AES-256 file encryption, file hash, **TOTP/2FA code generator** (RFC 6238, live countdown — secret never uploaded), **PII redactor** (mask emails/phones/SSNs/cards[Luhn]/IPs/IBANs in text before pasting into AI — 100% client-side) |
