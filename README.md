...
| [Home & DIY](https://lazytools.io/home/) | **21 tools** — **brick & block** (count per wall + mortar joint + waste), **excavation** (volume, soil weight, swell, truckloads), **rebar** (bars each way, length, weight by ASTM size, tie-wire count), **roof pitch** (rise/run → x-in-12, degrees, slope %, rafter length), **stair calculator** (rise/run/stringer + IRC limits), **wall stud calculator** (16/24″ OC + plates), **BTU / AC sizing** (ENERGY STAR), — **paint**, **tile & flooring**, **concrete** (+ pre-mix bags), **mulch & soil**, **wallpaper**, **gravel & aggregate** (volume + tonnes), **drywall/plasterboard** (sheets), **roofing** (squares + shingle bundles via pitch), **sod & grass seed**, **fence** (posts/panels/rails), **deck boards**, **board-foot** lumber — metric or imperial |
